...
- GelSight Size-Buckets Progress 是不覆盖旧 task 的奖励对照：Teacher/Student 分别使用 `...-Progress-Teacher-v0` 与 `...-Progress-Student-DR-v0`。Reach proximity、归一化 lift 和接触奖励都只支付有符号状态进展，回退会扣回先前 shaping；成功高度连续保持 5 步时发放一次 `+100` 并返回 terminated，timeout 仍只 truncated。动作幅值按环境动作尺度归一化后每步 `-0.05 mean(a²)`；任一 GelPad 超过 15 N 后按最大侧归一化超量施加权重 5 的二次惩罚。观测与 4 维动作 shape 不变，但奖励/done 与旧 task 的 checkpoint 语义隔离。
