...
- `source/tacex_tasks/tacex_tasks/sim2real_gelsight_rma` 是 RMA 的 GelSight robot profile 薄封装；已注册 task 统一加载持久组合资产 `franka_gsmini_standard_arm_visuals_v7.usd`，Franka 根位姿统一为 `(0,0,0.015) m`。v7 保持 `panda_hand` 与 `panda_link8` 原点重合，只将左右 finger、GelSight case/GelPad 和中心指尖参考相对原始 v5 资产沿 hand 局部 `+Z` 延长 `21 mm`（即较 v6 向上回移 5 mm）；hand→GelPad 中心/最低点统一为 `0.1392/0.1563 m`，IK、reach/critic/safety 和 Teacher FK 使用同一几何。外部相机继续保持原 base_T_camera 标定。全部 GelSight task 以 Cube→`gelpad_left/right` 每策略步最大过滤力生成标签：每侧严格 `>2 N` 算接触，单侧/双侧奖励为 `+1.5/+3.0`；任一侧严格 `>15 N` 每步惩罚 `-5`。Cube 相对本 episode 稳态高度曾抬升 `>=20 mm` 后回落到 `<5 mm` 时，一次性惩罚 `-10` 且不终止。GelSight `tactile_rgb` 不进入 Teacher Actor observation，Teacher 仍是 `proprio[15]+history[4]+cube_xyz[3]+contact[2]`；Student 使用第三视角 RGB 预测 Cube XYZ、左右 tactile RGB 预测 contact。
- GelSight Size-Buckets 是独立契约：8 个 4–6 cm 等间距尺寸按 `env_id % 8` 固定，一 env 一 Cube、reset 不缩放，环境数必须为 8 的倍数；3.5 m spacing、不可见共享 GroundPlane 和显式 collision filtering 隔离环境。Cube—非 gelpad 机器人或机器人—桌面统一为 `illegal_collision`，惩罚阈值在0–100k policy step从20 N线性降至5 N、权重固定 -10，`>10 N` 独立 terminated，Cube—桌面中性，5 s timeout 只 truncated。当前 Teacher manifest/Student checkpoint 为 v6/v7，环境 profile 为 v5。
- GelSight Size-Buckets Progress 是不覆盖旧 task 的奖励对照：Teacher/Student 分别使用 `...-Progress-Teacher-v0` 与 `...-Progress-Student-DR-v0`。Reach proximity、归一化 lift 和接触奖励都只支付有符号状态进展，回退会扣回先前 shaping；成功高度连续保持 5 步时，done 先提交的 hold counter 在同一 transition 产生一次 `+100` 和 terminated，timeout 仍只 truncated。动作幅值按环境动作尺度归一化后每步 `-0.05 mean(a²)`；任一 GelPad 超过 15 N 后按最大侧归一化超量施加权重 5 的二次惩罚。Progress profile/Teacher/Student 分别为 v6/v7/v8；非 Progress v5/v6/v7 保持不变。
- GelSight X040-DR Size-Buckets Teacher/三帧 Student 同样使用一 env 一 Cube：启动前按 `env_id % 8` 固定 4–6 cm 尺寸，reset 保持尺寸并继续采样 `x=0.40±0.08 m/y=±0.10 m` 与7臂关节截断高斯噪声。Student 的位置辅助头使用 X040-Wide robot-root XYZ 归一化，中心为 `[0.40,0,0.026] m`、尺度为 `[0.08,0.10,0.10] m`；ResNet18 必须从经 artifact 校验的 RMA XY Heatmap-DR Student encoder 初始化。RTX 渲染以 `render_interval=decimation=2` 对齐30 Hz策略步；左右内部 depth camera 不回读 latest pose，Taxim `[0,1]` 输出固定乘255转 `uint8`。异构碰撞几何采用 `replicate_physics=false`、3.5 m spacing、可见黑色共享 GroundPlane、显式跨 env 过滤和 GPU dynamics；`floor_panel` 为 `3.5×3.5 m`，visual-only backdrop 同宽。该路线与其他 GelSight task 一样加载统一 v7 USD。Teacher manifest/Student checkpoint 为 v7/v10；illegal-collision 惩罚阈值仍在0–100k从100 N收紧至10 N、权重为 -10，但接触力不触发 terminated；机器人刚体原点穿过 ground 仍 terminated，timeout 仍只 truncated。观测、4维动作和 success 语义不变。
- GelSight X040 Progress 是隔离的新配对 task：场景、Cube `x=0.40±0.08 m/y=±0.10 m`、七臂关节截断高斯 reset 噪声、三帧七路 Student 输入和 full-strength DR 与 X040-DR 路线一致，继续使用 v7 21 mm USD。Reach 每步使用权重2.5的绝对距离 proximity，lift 每步使用权重2.5的 reset-relative 归一化绝对高度，contact 使用 signed progress；同时采用20→5 N非法碰撞惩罚、归一化动作幅值/变化率惩罚和15 N二次过力惩罚。相对初始高度抬升35 mm并连续保持5步时，在同一 transition 一次性 `+1000` 且 terminated/reset。Progress profile/Teacher/Student 为 v6/v8/v11；非 Progress v7/v7/v10 保持不变。
- X040 Progress Binary-Tactile 是新增的 Student-only 对照 task，复用上述 Progress Teacher、MDP 和 full-strength DR。环境继续输出左右 current/reference `uint8 RGB[96,128,3]`；Student/TorchScript 内部按像素计算 RGB 三通道绝对差值的最大值，严格 `>5` 后得到 `float32[1,96,128]` 的 0/1 mask，再由独立的一通道共享 CNN 编码。七路运行时输入与三路输出保持不变，但 model/input artifact 契约与 signed-delta Student 隔离，必须重新蒸馏。
- 双 GelSight Pulled-Drawer 是与 X040 隔离的独立 profile，并按 `occluded_grasping` 的抽屉拓扑建模：Franka 根位姿为 `(0,0,0.015) m`；白色外柜为开口朝 `-X` 的四板壳体，拉出的抽屉为 8 mm 底板及四壁组成的顶部开口刚体布局。每个 env 在 PhysX 启动前按 seed 采样一个共享 isotropic scale `0.9–1.1×` 并同时应用到外柜/内抽屉 XYZ，另采样柜体 XY `±0.02 m` 与抽屉 Y `±0.02 m`；抽屉 X 由零接缝约束计算，标称中心为 `(0.5275,0) m`，此后 reset 不改几何。Cube 的 4–6 cm 八档尺寸在每组8个 env 内 seeded permutation 且固定，episode reset 相对抽屉中心采样 XY `±0.03 m`，Z 为底板顶面加 Cube 半边长。Student 逐 env reset 更新外柜/抽屉颜色和左右 GelSight reference，但 opacity 固定 `1.0`。Cube—抽屉与 Cube—GelPad 合法，机器人—外柜/抽屉及 Cube—外柜进入渐进碰撞惩罚与强碰撞终止；板件和机器人 solver 保持 `32/4`。Pulled-Drawer 复用全局 v7 USD 和 `0.1392/0.1563 m` 末端偏移；Teacher 输入、Student 七路输入及三路输出不变。geometry 仍为 v10，Teacher manifest/Student artifact 为 v11，Teacher model/Student model 为 v5/v4、normalizer 为 v3；旧完整 checkpoint fail-closed，仅允许显式复用经校验的旧视觉 encoder 初始化。
- Pulled-Drawer Progress Binary-Tactile 是不覆盖旧路线的新 Teacher–Student 配对：完全继承抽屉的固定几何实例、8档 Cube 和 Student 外观 DR，使用 X040 Progress 的绝对 reach、绝对 lift、signed contact progress、同帧一次性 `+1000` 终止成功奖励与 15 N 二次过力惩罚，并显式保留抽屉 `200→20 N` 强碰撞终止。Teacher Actor 仍为抽屉 normalizer 的 30 维输入；Student 内部以严格 `>5` 的单通道二值触觉代替 signed delta，对外仍为七输入/三输出。环境 profile 为 `rma_gelsight_pulled_drawer_progress_v2`，Teacher/Student artifact 各用独立 v2 kind/version，新 Teacher 需重训并重新蒸馏 Student。
- Four-Tactile Pulled-Drawer Progress 使用独立 V8 资产，在 V7 两内侧 GelSight 基础上增加左右下向 GelSight；两个下向 case、Camera/plate 子树和 GelPad 弹性体作为完整组件绕各自 case 原点旋转90度，FixedJoint 同步重定向。Teacher 接触顺序为 `[left_inner,right_inner,left_down,right_down]`，Actor 为32维；Student 为三帧视觉加四组 current/reference 的11输入，输出 action4/contact4/cube_xyz3，融合维度1555。内侧和下向各自左右共享 CNN、方向间不共享；接触 signed progress 权重为 `1.5/1.5/1.0/1.0`，四路 `>15 N` 使用最大超额二次惩罚。下向 GelPad 不再设置独立阈值、固定惩罚或终止，只保留四路各自的原始接触力及下向最大力诊断。通用非法碰撞仍按 `20→5 N` 阈值每步惩罚 `-10`，但不再按 `200→20 N` 强碰撞阈值提前 done。TCP/reach 仍取内侧中点，clearance 对旋转后的下向 GelPad 底面8个 hand-frame 点变换后取最低 world-Z（最远 hand-Z `0.18331 m`）；环境/Teacher manifest/Student checkpoint profile 为 v6，模型结构版本不变，其余抽屉 Progress MDP 和成功终止不变。
- High-Cylinder Four-Tactile Pulled-Drawer 是与 Cube 隔离的独立 Teacher–Student 路线：目标为标称高 `0.100 m`、半径 `0.025 m`、密度 `1000 kg/m³` 的圆柱，按 `[0.90,0.93,0.96,0.99,1.01,1.04,1.07,1.10]` 做 isotropic scale；每组8个 env 由 seed 均衡排列并在 PhysX 启动前固定，实际高 `90–110 mm`、直径 `45–55 mm`、质量约 `143–261 g`，初始质心 Z 为抽屉底板顶面加实际半高。内部保留 `/cube`、`_cube`、`rma_cube_pos` 兼容接口，圆柱最低点使用有限圆柱的轴向半高与径向投影计算；成功为质心相对 reset 抬升 `35 mm` 并保持5步，不限制倾角。圆柱专用四触觉奖励按每步绝对接触状态计算：`0.2 × [1.5,1.5,1.0,1.0]`，满接触上限 `+1.0`，Cube 路线仍使用接触变化量。Teacher Actor 仍为32维，Student 仍为11输入/1555维融合和4维动作；环境 profile 为 `rma_gelsight_pulled_drawer_progress_four_tactile_cylinder_v2`，Teacher/Student 使用独立 artifact kind，拒绝 Cube checkpoint。
- Large-Pulled-Drawer High-Cylinder 是上述圆柱路线的独立几何变体：柜体/托盘标称外尺寸分别为 `400×350×180 mm` 与 `250×320×150 mm`（深×宽×高），共享 `0.9–1.1×` 每 env 固定缩放和原有位置扰动。柜体标称中心后移到 `(0.8525,-0.01) m`，使零接缝托盘标称中心继续保持 `(0.5275,0) m`；圆柱分桶不变，每 episode 相对实际托盘中心做 X/Y 各 `±50 mm` reset。观测/动作/奖励/done 均不变；新 task 使用独立 v2 profile/artifact，避免与标准抽屉及早期 `±30 mm` 大抽屉 checkpoint 混用。
- Large-Pulled-Drawer High-Cylinder 的六策略融合消融共享同一标准 Teacher 和物理 MDP。六个 Student 都在颜色/内参视觉 DR 后、传感器噪声和 `uint8` history 写入前固定执行 `bilinear 224→32→224`，关闭 Gaussian blur，history 保持 `[N,3,224,224,3]`；原独立 Downsample Teacher task 仍保留，但不用于这组对比。Vision-Only 运行接口不接触觉；VT 保留 1555 维 concat；Cross 系列由四个触觉 token 查询三个带时序位置编码的视觉 token，Alpha 由本体状态或“预测遮挡率+四路二值触觉面积+本体状态”产生；GRU 版本显式传入 `[N,4,9,256]` 历史并返回下一状态。六种 Student artifact identity 相互隔离，标准 Teacher checkpoint 可复用，所有 Student 均需重新蒸馏。
- 配套数据链路先按多个 geometry seed 扫描当前相机的 Replicator bbox occlusion，再按 geometry seed 隔离训练/验证/测试来拟合 `XY→64→64→occlusion` 伪标签器；伪标签器只在 Aux Student 蒸馏期使用，部署策略仅使用其视觉遮挡头。联合评测保存统一 XY/尺寸桶/geometry-seed/reset-seed schedule，逐一回放六个 checkpoint，并用每次实际 reset 后的 Replicator bbox occlusion 分桶，报告 `[0.0,0.1)` 至 `[0.6,0.7)`、`[0.7,1.0]` 成功率、Wilson 95% 区间、总体 micro、跨非空桶 macro、四路接触率、alpha/遮挡预测、参数量及模型前向延迟。
