...
- GelSight Size-Buckets 是独立的新契约：8 个 4–6 cm 等间距尺寸按 `env_id % 8` 固定，一 env 一 Cube、reset 不缩放，环境数必须为 8 的倍数；3.5 m spacing、不可见共享 GroundPlane 和显式 collision filtering 隔离环境。gelpad `>0.2 N` 生成左右二值接触并给予单侧 +1.5/双侧 +3；Cube—非 gelpad 机器人或机器人—桌面统一为 `illegal_collision`，惩罚阈值在0–100k policy step从20 N线性降至5 N、权重固定 -10，`>10 N` 独立 terminated，Cube—桌面中性，5 s timeout 只 truncated。
- GelSight X040-DR Size-Buckets Teacher/三帧 Student 同样使用一 env 一 Cube：启动前按 `env_id % 8` 固定 4–6 cm 尺寸，reset 保持尺寸并继续采样 `x=0.40±0.08 m/y=±0.10 m` 与7臂关节截断高斯噪声。Student 的位置辅助头使用 X040-Wide robot-root XYZ 归一化，中心为 `[0.40,0,0.026] m`、尺度为 `[0.08,0.10,0.10] m`；ResNet18 必须从经 artifact 校验的 RMA XY Heatmap-DR Student encoder 初始化，不再以 ImageNet 权重直接开始蒸馏。GelSight config mixin 显式转发 Real-Alignment `__post_init__`，因此 RTX 渲染以 `render_interval=decimation=2` 对齐30 Hz策略步；当前 RMA 路线没有 marker simulator，左右内部 depth camera 不回读 latest pose，Taxim `[0,1]` 浮点输出以固定255尺度转换为 `uint8`。异构碰撞几何采用 `replicate_physics=false`、3.5 m spacing、可见黑色共享 GroundPlane、显式跨 env 过滤和 GPU dynamics；每个 `floor_panel` 为 `3.5×3.5 m` 并从 backdrop 的 `x=-0.32 m` 开始，相邻 cell 边界严格相接，visual-only backdrop 的宽度同为3.5 m。X040 使用运行时资产根生成并缓存独立组合 USD：根引用原 GelSight 物理资产，将旧 Franka arm link0–7 `visuals` 设为 inactive，并以独立 `standard_visuals` 引用 Isaac Lab 标准 Panda；仅 `panda_link0/standard_visuals/panda_link0/subset_5` 绑定固定纯绿色 UsdPreviewSurface 自发光材质，腕部 LED 保留标准蓝色；hand/finger、GelSight case/gelpad visual、额外刚体、碰撞、关节和自碰撞全部保留，不做场景内逐 prim reference 改写。该视觉契约对应 Student checkpoint v6；v5 仅允许 play/export，不允许 resume。该路线不再创建八 Cube collection、selected-cube adapter、八组接触传感器或停车区。illegal-collision 惩罚阈值仍在0–100k从20 N收紧至5 N，终止阈值从200 N收紧至20 N；观测、4维动作、奖励权重和 success/done 不变。
