...
- 新 Student 每个 episode 在首个 post-reset 触觉更新时分别锁存左右 reference，局部 reset 不修改其他 env；共享单侧触觉编码器输入 `(current-reference)/255`，BCE 训练 logits，冻结 Actor 接收 `logit>=0` 的硬二值。部署输入固定为 `wrist_rgb+proprio+history+left_current+right_current+left_reference+right_reference`，输出 `action[4]`。
