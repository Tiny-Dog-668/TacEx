...
- Four-Tactile Pulled-Drawer Progress 使用独立 V8 资产，在 V7 两内侧 GelSight 基础上增加左右下向 GelSight。Teacher 接触顺序为 `[left_inner,right_inner,left_down,right_down]`，Actor 为32维；Student 为三帧视觉加四组 current/reference 的11输入，输出 action4/contact4/cube_xyz3，融合维度1555。内侧和下向各自左右共享 CNN、方向间不共享；接触 signed progress 权重为 `1.5/1.5/1.0/1.0`，四路 `>15 N` 使用最大超额二次惩罚；下向 GelPad 额外每步 `-10` 的严格超限阈值随全局 policy step `0→100k` 从 `50→10 N` 线性收紧但不终止，并复用碰撞课程的 resume offset。TCP/reach 仍取内侧中点，clearance 对下向 GelPad 底面8个 hand-frame 点变换后取最低 world-Z（最远 hand-Z `0.18331 m`）；环境/Teacher manifest/Student checkpoint profile 为 v2，模型结构版本不变，其余抽屉 Progress MDP、成功和强碰撞终止不变。
