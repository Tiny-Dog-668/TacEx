...
- GelSight X040-DR Size-Buckets Teacher/三帧 Student 同样使用一 env 一 Cube：启动前按 `env_id % 8` 固定 4–6 cm 尺寸，reset 保持尺寸并继续采样 `x=0.40±0.08 m/y=±0.10 m` 与7臂关节截断高斯噪声。异构碰撞几何采用 `replicate_physics=false`、3.5 m spacing、隐藏共享 GroundPlane、显式跨 env 过滤和 GPU dynamics；不再创建八 Cube collection、selected-cube adapter、八组接触传感器或停车区。illegal-collision 惩罚阈值仍在0–100k从20 N收紧至5 N，终止阈值从200 N收紧至20 N；观测、4维动作、奖励权重和 success/done 不变。底座 LED 不参加该 Student 的 DR：当前 GelSight robot USD 的 visual branch 不可安全地逐 env 解绑，强行修改会导致 Kit 在场景创建期退出。
