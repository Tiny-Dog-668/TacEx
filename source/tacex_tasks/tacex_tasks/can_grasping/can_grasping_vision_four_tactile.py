# Copyright (c) 2022-2025, The Isaac Lab Project Developers.
# All rights reserved.
#
# SPDX-License-Identifier: BSD-3-Clause

"""Can Grasping Environment with Privileged State Observations plus Wrist Camera."""

from __future__ import annotations

import os
import torch
import torch.nn as nn
import torch.nn.functional as F

# for Domain Randomization
import isaaclab.envs.mdp as mdp
import isaaclab.sim as sim_utils
import isaaclab.utils.math as math_utils
from isaaclab.assets import Articulation, ArticulationCfg, AssetBaseCfg, RigidObject, RigidObjectCfg
from isaaclab.controllers.differential_ik import DifferentialIKController
from isaaclab.controllers.differential_ik_cfg import DifferentialIKControllerCfg
from isaaclab.envs import DirectRLEnv, DirectRLEnvCfg, ViewerCfg
from isaaclab.envs.ui import BaseEnvWindow
from isaaclab.managers import EventTermCfg as EventTerm
from isaaclab.managers import SceneEntityCfg
from isaaclab.markers import VisualizationMarkers
from isaaclab.markers.config import FRAME_MARKER_CFG
from isaaclab.markers.visualization_markers import VisualizationMarkersCfg
from isaaclab.scene import InteractiveSceneCfg
from isaaclab.sensors import FrameTransformer, FrameTransformerCfg, TiledCamera, TiledCameraCfg
from isaaclab.sensors.frame_transformer.frame_transformer_cfg import OffsetCfg
from isaaclab.sim import PhysxCfg, SimulationCfg
from isaaclab.sim.schemas.schemas_cfg import RigidBodyPropertiesCfg
from isaaclab.sim.spawners.from_files.from_files import _spawn_from_usd_file
from isaaclab.utils import configclass
from isaaclab.utils.assets import ISAAC_NUCLEUS_DIR
from isaaclab.utils.math import (
    euler_xyz_from_quat,
    quat_error_magnitude,
    sample_uniform,
    wrap_to_pi,
)
from isaaclab.utils.noise import (
    GaussianNoiseCfg,
    NoiseModelCfg,
    UniformNoiseCfg,
    gaussian_noise,
)
import isaacsim.core.utils.prims as prim_utils
from pxr import UsdGeom
try:
    import torchvision
    from torchvision.models import ResNet18_Weights
    _HAS_TORCHVISION = True
except Exception:
    _HAS_TORCHVISION = False

from tacex import GelSightSensor
from tacex_assets import TACEX_ASSETS_DATA_DIR
from tacex_assets.robots.franka.franka_gsmini_gripper_rigid import (
    FRANKA_PANDA_ARM_GSMINI_GRIPPER_HIGH_PD_RIGID_CFG,
)
from tacex_assets.sensors.gelsight_mini.gsmini_cfg import GelSightMiniCfg

from tacex_tasks.utils import DirectLiveVisualizer
from .can_grasping_vision_only_resnet18 import (
    CanGraspingVisionOnlyCfg as CanGraspingVisionOnlyResNet18Cfg,
    CanGraspingVisionOnlyEnv as CanGraspingVisionOnlyResNet18Env,
)

YCB_CAN_USD_PATH = "/home/tinydog/Projects/ycb-tools/models/ycb/002_master_chef_can/can.usda"
YCB_CAN_SCALE = (0.5, 0.5, 0.5)


@sim_utils.clone
def spawn_ycb_can(
    prim_path: str,
    cfg: sim_utils.UsdFileCfg,
    translation: tuple[float, float, float] | None = None,
    orientation: tuple[float, float, float, float] | None = None,
):
    spawn_cfg = cfg.replace(rigid_props=None, collision_props=None, mass_props=None)
    prim = _spawn_from_usd_file(prim_path, spawn_cfg.usd_path, spawn_cfg, translation, orientation)

    if cfg.rigid_props is not None:
        sim_utils.define_rigid_body_properties(prim_path, cfg.rigid_props)
    else:
        sim_utils.define_rigid_body_properties(prim_path, sim_utils.RigidBodyPropertiesCfg())

    if cfg.collision_props is not None:
        mesh_prims = sim_utils.get_all_matching_child_prims(
            prim_path, predicate=lambda prim: prim.IsA(UsdGeom.Mesh)
        )
        if not mesh_prims:
            raise RuntimeError(f"No mesh prims found under '{prim_path}' for collision.")
        for mesh_prim in mesh_prims:
            sim_utils.define_collision_properties(mesh_prim.GetPath().pathString, cfg.collision_props)

    if cfg.mass_props is not None:
        sim_utils.define_mass_properties(prim_path, cfg.mass_props)

    return prim


class CustomEnvWindow(BaseEnvWindow):
    """Window manager for the RL environment."""

    def __init__(self, env: DirectRLEnvCfg, window_name: str = "IsaacLab"):
        """Initialize the window.

        Args:
            env: The environment object.
            window_name: The name of the window. Defaults to "IsaacLab".
        """
        # initialize base window
        super().__init__(env, window_name)
        # add custom UI elements
        with self.ui_window_elements["main_vstack"]:
            with self.ui_window_elements["debug_frame"]:
                with self.ui_window_elements["debug_vstack"]:
                    # add command manager visualization
                    self._create_debug_vis_ui_element("targets", self.env)


@configclass
class CanGraspingVisionFourTactileCfg(CanGraspingVisionOnlyResNet18Cfg):
    """Configuration for can grasping with wrist-camera vision and 4 tactile RGB sensors."""

    tactile_img_res_hw = (96, 128)
    tactile_diag_enabled = True
    tactile_diag_diff_res_hw = (24, 32)
    tactile_diag_activity_threshold = 0.01
    tactile_diag_print_interval = 500

    # Use the 4-pad GelSight Mini asset (inner left/right + left_down/right_down pads).
    robot: ArticulationCfg = FRANKA_PANDA_ARM_GSMINI_GRIPPER_HIGH_PD_RIGID_CFG.replace(
        prim_path="/World/envs/env_.*/Robot",
        spawn=FRANKA_PANDA_ARM_GSMINI_GRIPPER_HIGH_PD_RIGID_CFG.spawn.replace(
            usd_path=f"{TACEX_ASSETS_DATA_DIR}/Robots/Franka/GelSight_Mini/Gripper/physx_rigid_gelpads.down.usda"
        ),
        init_state=ArticulationCfg.InitialStateCfg(
            joint_pos={
                "panda_joint1": -0.3136,
                "panda_joint2": -0.4448,
                "panda_joint3": 0.3858,
                "panda_joint4": -3.0236,
                "panda_joint5": 0.2319,
                "panda_joint6": 2.5776,
                "panda_joint7": 0.6249,
                "panda_finger_joint.*": 0.02,
            },
            pos=(0.0, 0.0, 0.0),
            rot=(1.0, 0.0, 0.0, 0.0),
        ),
    )

    # Four tactile RGB sensors (inner left/right + left_down/right_down).
    gsmini_left: GelSightMiniCfg = GelSightMiniCfg(prim_path="/World/envs/env_.*/Robot/gelsight_mini_case_left")
    gsmini_left.sensor_camera_cfg = GelSightMiniCfg.SensorCameraCfg(
        prim_path_appendix="/Camera",
        update_period=2,
        resolution=(128, 96),
        data_types=["depth"],
        clipping_range=(0.024, 0.034),
    )
    gsmini_left.data_types = ["camera_depth", "tactile_rgb"]
    gsmini_left.marker_motion_sim_cfg = None
    gsmini_left.optical_sim_cfg = gsmini_left.optical_sim_cfg.replace(
        tactile_img_res=(tactile_img_res_hw[1], tactile_img_res_hw[0])
    )

    gsmini_right: GelSightMiniCfg = GelSightMiniCfg(prim_path="/World/envs/env_.*/Robot/gelsight_mini_case_right")
    gsmini_right.sensor_camera_cfg = GelSightMiniCfg.SensorCameraCfg(
        prim_path_appendix="/Camera",
        update_period=2,
        resolution=(128, 96),
        data_types=["depth"],
        clipping_range=(0.024, 0.034),
    )
    gsmini_right.data_types = ["camera_depth", "tactile_rgb"]
    gsmini_right.marker_motion_sim_cfg = None
    gsmini_right.optical_sim_cfg = gsmini_right.optical_sim_cfg.replace(
        tactile_img_res=(tactile_img_res_hw[1], tactile_img_res_hw[0])
    )

    gsmini_left_down: GelSightMiniCfg = GelSightMiniCfg(
        prim_path="/World/envs/env_.*/Robot/gelsight_mini_case_left_down"
    )
    gsmini_left_down.sensor_camera_cfg = GelSightMiniCfg.SensorCameraCfg(
        prim_path_appendix="/Camera",
        update_period=2,
        resolution=(128, 96),
        data_types=["depth"],
        clipping_range=(0.024, 0.034),
    )
    gsmini_left_down.data_types = ["camera_depth", "tactile_rgb"]
    gsmini_left_down.marker_motion_sim_cfg = None
    gsmini_left_down.optical_sim_cfg = gsmini_left_down.optical_sim_cfg.replace(
        tactile_img_res=(tactile_img_res_hw[1], tactile_img_res_hw[0])
    )

    gsmini_right_down: GelSightMiniCfg = GelSightMiniCfg(
        prim_path="/World/envs/env_.*/Robot/gelsight_mini_case_right_down"
    )
    gsmini_right_down.sensor_camera_cfg = GelSightMiniCfg.SensorCameraCfg(
        prim_path_appendix="/Camera",
        update_period=2,
        resolution=(128, 96),
        data_types=["depth"],
        clipping_range=(0.024, 0.034),
    )
    gsmini_right_down.data_types = ["camera_depth", "tactile_rgb"]
    gsmini_right_down.marker_motion_sim_cfg = None
    gsmini_right_down.optical_sim_cfg = gsmini_right_down.optical_sim_cfg.replace(
        tactile_img_res=(tactile_img_res_hw[1], tactile_img_res_hw[0])
    )

    observation_space = {
        "proprio_obs": 18,
        "gripper_state": 1,
        "action_history": 5,
        "wrist_resnet": 512,
        "tactile_left_resnet": 128,
        "tactile_right_resnet": 128,
        "tactile_left_down_resnet": 128,
        "tactile_right_down_resnet": 128,
        "critic_can_pos": 3,
        "critic_can_quat": 4,
        "critic_can_lin_vel": 3,
        "critic_can_ang_vel": 3,
        "critic_gripper_pos": 3,
        "critic_gripper_quat": 4,
        "critic_gripper_lin_vel": 3,
        "critic_gripper_ang_vel": 3,
        "critic_target_pos": 3,
        "critic_target_distance": 1,
    }


class CanGraspingVisionFourTactileEnv(CanGraspingVisionOnlyResNet18Env):
    """Can grasping environment with wrist-camera vision and 4 tactile RGB sensors."""

    cfg: CanGraspingVisionFourTactileCfg

    def __init__(self, cfg: CanGraspingVisionFourTactileCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)

        self._tactile_feature_dim = 128
        if _HAS_TORCHVISION:
            resnet = torchvision.models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
            resnet.fc = nn.Linear(resnet.fc.in_features, self._tactile_feature_dim)
            self._tactile_encoder = resnet.to(self.device)
            trainable = not bool(self.cfg.resnet18_frozen)
            self._tactile_encoder.train(trainable)
            for p in self._tactile_encoder.parameters():
                p.requires_grad_(trainable)
            self._tactile_imgnet_mean = torch.tensor([0.485, 0.456, 0.406], device=self.device).view(1, 3, 1, 1)
            self._tactile_imgnet_std = torch.tensor([0.229, 0.224, 0.225], device=self.device).view(1, 3, 1, 1)
        else:
            self._tactile_encoder = None
            self._tactile_imgnet_mean = None
            self._tactile_imgnet_std = None

        self._tactile_diag_enabled = bool(getattr(self.cfg, "tactile_diag_enabled", True))
        self._tactile_diag_activity_threshold = float(getattr(self.cfg, "tactile_diag_activity_threshold", 0.01))
        self._tactile_diag_activity_threshold = max(0.0, self._tactile_diag_activity_threshold)
        self._tactile_diag_print_interval = int(
            getattr(self.cfg, "tactile_diag_print_interval", self.reward_print_interval)
        )
        self._tactile_sensor_names = ("left", "right", "left_down", "right_down")
        diag_h, diag_w = getattr(self.cfg, "tactile_diag_diff_res_hw", (24, 32))
        self._tactile_diag_diff_h = max(int(diag_h), 1)
        self._tactile_diag_diff_w = max(int(diag_w), 1)
        self._tactile_prev_diag_gray = torch.zeros(
            (
                self.num_envs,
                len(self._tactile_sensor_names),
                self._tactile_diag_diff_h,
                self._tactile_diag_diff_w,
            ),
            device=self.device,
            dtype=torch.float32,
        )
        self._tactile_prev_valid = torch.zeros(
            (self.num_envs, len(self._tactile_sensor_names)), device=self.device, dtype=torch.bool
        )
        self._tactile_ep_info_sum = torch.zeros(
            (self.num_envs, len(self._tactile_sensor_names)), device=self.device, dtype=torch.float32
        )
        self._tactile_ep_info_count = torch.zeros_like(self._tactile_ep_info_sum)
        self._tactile_ep_std_sum = torch.zeros_like(self._tactile_ep_info_sum)
        self._tactile_ep_std_count = torch.zeros_like(self._tactile_ep_info_sum)
        self._tactile_window_info_sum = torch.zeros(
            (len(self._tactile_sensor_names),), device=self.device, dtype=torch.float32
        )
        self._tactile_window_std_sum = torch.zeros_like(self._tactile_window_info_sum)
        self._tactile_window_episode_count = 0

    def _setup_scene(self):
        super()._setup_scene()

        # tactile sensors
        self.gsmini_left = GelSightSensor(self.cfg.gsmini_left)
        self.scene.sensors["gsmini_left"] = self.gsmini_left
        self.gsmini_right = GelSightSensor(self.cfg.gsmini_right)
        self.scene.sensors["gsmini_right"] = self.gsmini_right
        self.gsmini_left_down = GelSightSensor(self.cfg.gsmini_left_down)
        self.scene.sensors["gsmini_left_down"] = self.gsmini_left_down
        self.gsmini_right_down = GelSightSensor(self.cfg.gsmini_right_down)
        self.scene.sensors["gsmini_right_down"] = self.gsmini_right_down

    def _normalize_tactile_rgb(
        self, rgb_tensor: torch.Tensor | None
    ) -> tuple[torch.Tensor | None, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Convert tactile RGB to [0, 1] and return std + low-res gray image for raw-diff diagnostics."""
        if rgb_tensor is None:
            zeros = torch.zeros((self.num_envs,), dtype=torch.float32, device=self.device)
            diag_gray = torch.zeros(
                (self.num_envs, self._tactile_diag_diff_h, self._tactile_diag_diff_w),
                dtype=torch.float32,
                device=self.device,
            )
            valid = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
            return None, zeros, diag_gray, valid

        x = rgb_tensor.to(device=self.device, dtype=torch.float32)
        max_val = x.max()
        if torch.isfinite(max_val) and max_val > 1.5:
            x = x / 255.0
        x = torch.nan_to_num(x, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
        per_env_std = x.std(dim=(1, 2, 3), unbiased=False)
        gray = (0.299 * x[..., 0] + 0.587 * x[..., 1] + 0.114 * x[..., 2]).unsqueeze(1)
        if gray.shape[2] != self._tactile_diag_diff_h or gray.shape[3] != self._tactile_diag_diff_w:
            gray = F.interpolate(
                gray,
                size=(self._tactile_diag_diff_h, self._tactile_diag_diff_w),
                mode="bilinear",
                align_corners=False,
            )
        diag_gray = gray.squeeze(1).contiguous()
        valid = torch.ones((self.num_envs,), dtype=torch.bool, device=self.device)
        return x, per_env_std, diag_gray, valid

    def _accumulate_tactile_episode_stats(self):
        tact_l_raw = getattr(self, "gsmini_left", None)
        tact_l_raw = tact_l_raw.data.output.get("tactile_rgb") if tact_l_raw is not None else None
        tact_r_raw = getattr(self, "gsmini_right", None)
        tact_r_raw = tact_r_raw.data.output.get("tactile_rgb") if tact_r_raw is not None else None
        tact_ld_raw = getattr(self, "gsmini_left_down", None)
        tact_ld_raw = tact_ld_raw.data.output.get("tactile_rgb") if tact_ld_raw is not None else None
        tact_rd_raw = getattr(self, "gsmini_right_down", None)
        tact_rd_raw = tact_rd_raw.data.output.get("tactile_rgb") if tact_rd_raw is not None else None

        _, tact_l_std, tact_l_diag, tact_l_valid = self._normalize_tactile_rgb(tact_l_raw)
        _, tact_r_std, tact_r_diag, tact_r_valid = self._normalize_tactile_rgb(tact_r_raw)
        _, tact_ld_std, tact_ld_diag, tact_ld_valid = self._normalize_tactile_rgb(tact_ld_raw)
        _, tact_rd_std, tact_rd_diag, tact_rd_valid = self._normalize_tactile_rgb(tact_rd_raw)

        tactile_stds = torch.stack((tact_l_std, tact_r_std, tact_ld_std, tact_rd_std), dim=1)
        tactile_diag_gray = torch.stack((tact_l_diag, tact_r_diag, tact_ld_diag, tact_rd_diag), dim=1)
        tactile_valid = torch.stack((tact_l_valid, tact_r_valid, tact_ld_valid, tact_rd_valid), dim=1)
        tactile_delta = torch.abs(tactile_diag_gray - self._tactile_prev_diag_gray).mean(dim=(2, 3))
        valid_delta = tactile_valid & self._tactile_prev_valid
        tactile_delta = torch.where(valid_delta, tactile_delta, torch.zeros_like(tactile_delta))
        tactile_info = valid_delta & (tactile_delta > self._tactile_diag_activity_threshold)

        self._tactile_ep_info_sum += tactile_info.float()
        self._tactile_ep_info_count += valid_delta.float()
        self._tactile_ep_std_sum += tactile_stds * tactile_valid.float()
        self._tactile_ep_std_count += tactile_valid.float()

        self._tactile_prev_diag_gray.copy_(tactile_diag_gray.detach())
        self._tactile_prev_valid.copy_(tactile_valid.detach())

    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        obs_dict = super()._get_observations()
        obs = obs_dict["policy"]

        target_h, target_w = getattr(self.cfg, "tactile_img_res_hw", (96, 128))

        def _prep_rgb_3ch(rgb_norm_hwc: torch.Tensor | None) -> torch.Tensor:
            if rgb_norm_hwc is None or self._tactile_encoder is None:
                return torch.zeros((self.num_envs, 3, target_h, target_w), dtype=torch.float32, device=self.device)
            xn = rgb_norm_hwc.permute(0, 3, 1, 2).contiguous()
            if xn.shape[2] != target_h or xn.shape[3] != target_w:
                xn = F.interpolate(xn, size=(target_h, target_w), mode="bilinear", align_corners=False)
            xn = (xn - self._tactile_imgnet_mean) / self._tactile_imgnet_std
            return xn

        tact_l_raw = getattr(self, "gsmini_left", None)
        tact_l_raw = tact_l_raw.data.output.get("tactile_rgb") if tact_l_raw is not None else None
        tact_r_raw = getattr(self, "gsmini_right", None)
        tact_r_raw = tact_r_raw.data.output.get("tactile_rgb") if tact_r_raw is not None else None
        tact_ld_raw = getattr(self, "gsmini_left_down", None)
        tact_ld_raw = tact_ld_raw.data.output.get("tactile_rgb") if tact_ld_raw is not None else None
        tact_rd_raw = getattr(self, "gsmini_right_down", None)
        tact_rd_raw = tact_rd_raw.data.output.get("tactile_rgb") if tact_rd_raw is not None else None

        tact_l_rgb, _, _, _ = self._normalize_tactile_rgb(tact_l_raw)
        tact_r_rgb, _, _, _ = self._normalize_tactile_rgb(tact_r_raw)
        tact_ld_rgb, _, _, _ = self._normalize_tactile_rgb(tact_ld_raw)
        tact_rd_rgb, _, _, _ = self._normalize_tactile_rgb(tact_rd_raw)

        tact_l = _prep_rgb_3ch(tact_l_rgb)
        tact_r = _prep_rgb_3ch(tact_r_rgb)
        tact_ld = _prep_rgb_3ch(tact_ld_rgb)
        tact_rd = _prep_rgb_3ch(tact_rd_rgb)

        if self._tactile_encoder is None:
            zeros = torch.zeros((self.num_envs, self._tactile_feature_dim), device=self.device, dtype=torch.float32)
            fl = fr = fld = frd = zeros
        else:
            dev = self.device
            dev_type = getattr(dev, "type", None)
            if dev_type is None:
                dev_type = "cuda" if (isinstance(dev, str) and dev.startswith("cuda")) else "cpu"
            use_amp = dev_type == "cuda"
            if self.cfg.resnet18_frozen:
                with torch.no_grad(), torch.amp.autocast("cuda", enabled=use_amp, dtype=torch.float16):
                    fl = self._tactile_encoder(tact_l).view(self.num_envs, self._tactile_feature_dim).float()
                    fr = self._tactile_encoder(tact_r).view(self.num_envs, self._tactile_feature_dim).float()
                    fld = self._tactile_encoder(tact_ld).view(self.num_envs, self._tactile_feature_dim).float()
                    frd = self._tactile_encoder(tact_rd).view(self.num_envs, self._tactile_feature_dim).float()
            else:
                with torch.amp.autocast("cuda", enabled=use_amp, dtype=torch.float16):
                    fl = self._tactile_encoder(tact_l).view(self.num_envs, self._tactile_feature_dim).float()
                    fr = self._tactile_encoder(tact_r).view(self.num_envs, self._tactile_feature_dim).float()
                    fld = self._tactile_encoder(tact_ld).view(self.num_envs, self._tactile_feature_dim).float()
                    frd = self._tactile_encoder(tact_rd).view(self.num_envs, self._tactile_feature_dim).float()

        obs.update(
            {
                "tactile_left_resnet": fl,
                "tactile_right_resnet": fr,
                "tactile_left_down_resnet": fld,
                "tactile_right_down_resnet": frd,
            }
        )

        return {"policy": obs}

    def _get_rewards(self) -> torch.Tensor:
        self._accumulate_tactile_episode_stats()
        rewards = super()._get_rewards()

        if self.reward_print_interval <= 0:
            return rewards
        if self.step_count % self.reward_print_interval != 0:
            return rewards

        if self._tactile_window_episode_count > 0:
            denom = float(self._tactile_window_episode_count)
            info_mean = self._tactile_window_info_sum / denom
            std_mean = self._tactile_window_std_sum / denom
        else:
            info_mean = torch.zeros_like(self._tactile_window_info_sum)
            std_mean = torch.zeros_like(self._tactile_window_std_sum)

        self.extras.setdefault("log", {})
        for idx, sensor_name in enumerate(self._tactile_sensor_names):
            self.extras["log"][f"tactile/{sensor_name}_info_ratio"] = info_mean[idx].detach()
            self.extras["log"][f"tactile/{sensor_name}_std"] = std_mean[idx].detach()

        if (
            self._tactile_diag_enabled
            and self._tactile_diag_print_interval > 0
            and self.step_count % self._tactile_diag_print_interval == 0
        ):
            parts = []
            for idx, sensor_name in enumerate(self._tactile_sensor_names):
                parts.append(f"{sensor_name}:info={info_mean[idx].item():.2f},std={std_mean[idx].item():.4f}")
            print(
                f"[TactileDiagWindow] step {self.step_count}: "
                f"episodes={self._tactile_window_episode_count} | "
                + " | ".join(parts)
            )

        self._tactile_window_info_sum.zero_()
        self._tactile_window_std_sum.zero_()
        self._tactile_window_episode_count = 0

        return rewards

    def _reset_idx(self, env_ids: torch.Tensor):
        has_diag_buffers = hasattr(self, "_tactile_ep_info_sum")
        if has_diag_buffers and env_ids.numel() > 0:
            ep_info_count = self._tactile_ep_info_count[env_ids]
            ep_std_count = self._tactile_ep_std_count[env_ids]
            finished_mask = (ep_info_count.sum(dim=1) > 0) | (ep_std_count.sum(dim=1) > 0)
            if finished_mask.any():
                done_env_ids = env_ids[finished_mask]
                info_sum = self._tactile_ep_info_sum[done_env_ids]
                info_count = self._tactile_ep_info_count[done_env_ids]
                std_sum = self._tactile_ep_std_sum[done_env_ids]
                std_count = self._tactile_ep_std_count[done_env_ids]
                info_mean = torch.where(
                    info_count > 0.0,
                    info_sum / info_count.clamp(min=1.0),
                    torch.zeros_like(info_sum),
                )
                std_mean = torch.where(
                    std_count > 0.0,
                    std_sum / std_count.clamp(min=1.0),
                    torch.zeros_like(std_sum),
                )
                self._tactile_window_info_sum += info_mean.sum(dim=0)
                self._tactile_window_std_sum += std_mean.sum(dim=0)
                self._tactile_window_episode_count += int(done_env_ids.numel())

        super()._reset_idx(env_ids)
        if env_ids.numel() == 0:
            return
        if has_diag_buffers:
            self._tactile_ep_info_sum[env_ids] = 0.0
            self._tactile_ep_info_count[env_ids] = 0.0
            self._tactile_ep_std_sum[env_ids] = 0.0
            self._tactile_ep_std_count[env_ids] = 0.0
        self._tactile_prev_diag_gray[env_ids] = 0.0
        self._tactile_prev_valid[env_ids] = False
