...
class CanGraspingVisionOnlyCfg(DirectRLEnvCfg):
    ...
    wrist_blur_kernel_size = 25
    wrist_blur_enabled = True
    wrist_gaussian_noise_std = 0.0
    # 亮度衰减：用于模拟现实黑暗环境（1.0=不变，0.0=最暗）
    wrist_brightness_enabled = True
    # 调暗方式：linear | gamma
    wrist_darkening_mode = "linear"
    # gamma 模式参数（建议 >= 1.0，越大越暗）
    wrist_gamma = 50
    # linear 模式参数（1.0=不变，0.0=全黑）
    wrist_brightness_factor = 0.1
    # 是否打印当前模式下的多档亮度统计
    wrist_mode_brightness_print = False
    # linear 模式导出图像参数（亮度比例）
    wrist_save_linear_levels = (0.8, 0.6, 0.4, 0.2, 0.01)
    # gamma 模式导出图像参数（gamma 值，>=1.0，越大越暗）
    wrist_save_gamma_levels = (10,20,30,40,50)
    wrist_save_images_enabled = True
    # 是否保存多档亮度(level)对比图
    wrist_save_level_images_enabled = False
    wrist_save_images_dir = "auto"
    wrist_save_images_max = 1
    ...
    # randomization
    can_pos_range = 0.10  # plate half-width for random placement
    can_rot_range = 0.0
...
class CanGraspingVisionOnlyEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CanGraspingVisionOnlyCfg, render_mode: str | None = None, **kwargs):
        ...
        if self._wrist_preprocess_enabled:
            ...
            blur_enabled = bool(getattr(self.cfg, "wrist_blur_enabled", True))
            if kernel_size > 1 and blur_enabled:
                if kernel_size % 2 == 0:
                    kernel_size += 1
                self._wrist_blur_padding = kernel_size // 2
                self._wrist_blur_kernel = self._build_box_kernel(kernel_size)
        self._wrist_brightness_enabled = bool(self.cfg.wrist_brightness_enabled)
        self._wrist_brightness_factor = float(self.cfg.wrist_brightness_factor)
        self._wrist_brightness_factor = max(0.0, min(self._wrist_brightness_factor, 1.0))
        self._wrist_darkening_mode = str(getattr(self.cfg, "wrist_darkening_mode", "linear")).strip().lower()
        if self._wrist_darkening_mode not in ("linear", "gamma"):
            print(
                f"[WARN] invalid wrist_darkening_mode='{self._wrist_darkening_mode}', "
                "fallback to 'linear'."
            )
            self._wrist_darkening_mode = "linear"
        self._wrist_gamma = float(getattr(self.cfg, "wrist_gamma", 2.2))
        if self._wrist_gamma < 1.0:
            print(f"[WARN] wrist_gamma={self._wrist_gamma} < 1.0, clamped to 1.0 for darkening.")
            self._wrist_gamma = 1.0
        self._wrist_mode_brightness_print = bool(getattr(self.cfg, "wrist_mode_brightness_print", True))
        self._wrist_save_linear_levels = self._sanitize_linear_levels(
            getattr(self.cfg, "wrist_save_linear_levels", (0.8, 0.6, 0.4, 0.2, 0.01))
        )
        self._wrist_save_gamma_levels = self._sanitize_gamma_levels(
            getattr(self.cfg, "wrist_save_gamma_levels", (1.2, 1.6, 2.2, 2.8, 3.5))
        )
        self._wrist_save_images_enabled = bool(self.cfg.wrist_save_images_enabled)
        self._wrist_save_level_images_enabled = bool(
            getattr(self.cfg, "wrist_save_level_images_enabled", True)
        )
    ...
    def _find_latest_skrl_run_dir(self) -> str:
        ...
        if env_dir:
            return env_dir
        log_root = os.path.abspath(os.path.join("logs", "skrl", "can_grasping"))
    ...
    def _sanitize_linear_levels(self, levels) -> tuple[float, ...]:
        if levels is None:
            return ()
        ...
        for item in raw_levels:
            ...
            if value < 0.0:
                continue
            parsed_levels.append(min(value, 1.0))
        if not parsed_levels:
            return ()
        return tuple(sorted(set(parsed_levels), reverse=True))

    def _sanitize_gamma_levels(self, levels) -> tuple[float, ...]:
        if levels is None:
            return ()
        if isinstance(levels, str):
            raw_levels = [item.strip() for item in levels.split(",") if item.strip()]
        ...
        if not parsed_levels:
            return ()
        return tuple(sorted(set(parsed_levels)))

    def _format_wrist_param_value(self, value: float) -> str:
        try:
            val = float(value)
        except (TypeError, ValueError):
            return "na"
    ...
    def _apply_linear_darkening(self, x: torch.Tensor, brightness_factor: float) -> torch.Tensor:
        x = torch.clamp(x, 0.0, 1.0)
        brightness_factor = max(0.0, min(brightness_factor, 1.0))
        return torch.clamp(x * brightness_factor, 0.0, 1.0)

    def _apply_gamma_darkening(self, x: torch.Tensor, gamma: float) -> torch.Tensor:
        x = torch.clamp(x, 0.0, 1.0)
        gamma = max(1.0, float(gamma))
        return torch.clamp(torch.pow(x, gamma), 0.0, 1.0)

    def _apply_wrist_brightness(self, x: torch.Tensor) -> torch.Tensor:
        if self._wrist_darkening_mode == "gamma":
            return self._apply_gamma_darkening(x, self._wrist_gamma)
        return self._apply_linear_darkening(x, self._wrist_brightness_factor)

    def _image_mean_luma(self, img_chw: torch.Tensor) -> float:
        # img_chw: [3, H, W], values in [0, 1]
        img = torch.clamp(img_chw.to(torch.float32), 0.0, 1.0)
        if img.ndim != 3 or img.shape[0] != 3:
            return float(img.mean().item())
    ...
    def _maybe_save_wrist_images(self, x_raw: torch.Tensor, x_proc: torch.Tensor):
        ...
        mode = self._wrist_darkening_mode
        current_param = self._wrist_gamma if mode == "gamma" else self._wrist_brightness_factor
        param_tag = self._format_wrist_param_value(current_param)
        base_tag = f"mode-{mode}_param-{param_tag}"
        raw_path = os.path.join(
            self._wrist_save_dir, f"wrist_raw_{base_tag}.png"
        )
        proc_path = os.path.join(
            self._wrist_save_dir, f"wrist_proc_{base_tag}.png"
        )
        torchvision.utils.save_image(raw, raw_path)
        torchvision.utils.save_image(proc, proc_path)

        mode_level_logs = []
        if self._wrist_save_level_images_enabled:
            if mode == "gamma":
                for gamma in self._wrist_save_gamma_levels:
                    dark_raw = self._apply_gamma_darkening(raw, gamma)
                    gamma_tag = self._format_wrist_param_value(gamma)
                    dark_raw_path = os.path.join(
                        self._wrist_save_dir,
                        f"wrist_raw_level_mode-gamma_param-{gamma_tag}.png",
                    )
                    torchvision.utils.save_image(dark_raw, dark_raw_path)
                    if self._wrist_mode_brightness_print:
                        mode_level_logs.append(f"gamma={float(gamma):.3f}:{self._image_mean_luma(dark_raw):.4f}")
            else:
                for level in self._wrist_save_linear_levels:
                    dark_raw = self._apply_linear_darkening(raw, level)
                    level_tag = self._format_wrist_param_value(level)
                    dark_raw_path = os.path.join(
                        self._wrist_save_dir,
                        f"wrist_raw_level_mode-linear_param-{level_tag}.png",
                    )
                    torchvision.utils.save_image(dark_raw, dark_raw_path)
                    if self._wrist_mode_brightness_print:
                        mode_level_logs.append(f"factor={float(level):.3f}:{self._image_mean_luma(dark_raw):.4f}")

        if self._wrist_mode_brightness_print:
            raw_luma = self._image_mean_luma(raw)
            proc_luma = self._image_mean_luma(proc)
            level_stats = ", ".join(mode_level_logs) if mode_level_logs else "none"
            if self._wrist_darkening_mode == "gamma":
                mode_param = f"wrist_gamma={self._wrist_gamma:.3f}"
            else:
                mode_param = f"wrist_brightness_factor={self._wrist_brightness_factor:.3f}"
            print(
                f"[Wrist Brightness] mode={self._wrist_darkening_mode} env={env_id} step={step_id} idx={idx} "
                f"{mode_param} raw={raw_luma:.4f} proc={proc_luma:.4f} levels({level_stats})"
            )
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        if hasattr(self, "_use_resnet18") and self._use_resnet18:
            ...
            if self._wrist_preprocess_enabled:
                ...
                x_proc = x
            if self._wrist_brightness_enabled:
                x_proc = self._apply_wrist_brightness(x_proc)
            if wrist_rgb_valid:
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if (
            ...
            can_height_mean = float(can_pos[:, 2].mean().item())
