...
@configclass
class CanGraspingVisionTwoTactileCfg(CanGraspingVisionOnlyResNet18Cfg):
    """Configuration for can grasping with wrist-camera vision and 2 tactile RGB sensors."""

    tactile_img_res_hw = (96, 128)
    tactile_valid_enable = True
    tactile_valid_diff_threshold = 0.02
    tactile_baseline_warmup_steps = 4
    tactile_baseline_retry_steps = 2
...
class CanGraspingVisionOnlyEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CanGraspingVisionOnlyCfg, render_mode: str | None = None, **kwargs):
        ...
        self._last_done_success = 0
        self._last_done_timeout = 0
        self._last_done_collision = 0
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if dropped_now.any():
            rewards = rewards + (-self.cfg.success_reward_weight) * dropped_now.float()
        ...
        if (
            ...
            print(
                ...
                f"done_envs={self._last_done_count} "
                f"(succ={self._last_done_success}, timeout={self._last_done_timeout}, "
                f"collision={self._last_done_collision})"
            )
    ...
    def _compute_success(self) -> torch.Tensor:
        ...
        return current_height > self.cfg.success_height
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if done_ids.numel() > 0:
            ...
            self.extras["log"]["success_rate"] = float(self._success_count) / max(self._episode_count, 1)
            self.extras["log"]["info/timeout_rate"] = float(time_out[done_ids].float().mean().item())
            self.extras["log"]["info/collision_rate"] = float(collision_with_ground[done_ids].float().mean().item())
            self.extras["log"]["info/success_rate_event"] = float(success[done_ids].float().mean().item())
            self._last_done_count = num_done
            self._last_done_success = num_success
            self._last_done_timeout = num_timeout
            self._last_done_collision = num_collision
        else:
            self._last_done_count = 0
            self._last_done_success = 0
            self._last_done_timeout = 0
...
class CanGraspingVisionTwoTactileEnv(CanGraspingVisionOnlyResNet18Env):
    """Can grasping environment with wrist-camera vision and 2 tactile RGB sensors."""

    cfg: CanGraspingVisionTwoTactileCfg

    def __init__(self, cfg: CanGraspingVisionTwoTactileCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)
        ...
        warmup_steps = int(getattr(self.cfg, "tactile_baseline_warmup_steps", 4))
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        tactile_valid = torch.zeros((self.num_envs, 1), device=self.device, dtype=torch.float32)
        tactile_score = torch.zeros((self.num_envs, 1), device=self.device, dtype=torch.float32)
        if bool(getattr(self.cfg, "tactile_valid_enable", True)):
            diff_thresh = float(getattr(self.cfg, "tactile_valid_diff_threshold", 0.02))
            score_l = (tact_l_01 - self._tactile_ref_left).abs().mean(dim=(1, 2, 3))
            score_r = (tact_r_01 - self._tactile_ref_right).abs().mean(dim=(1, 2, 3))
            score = torch.maximum(score_l, score_r)
            tactile_score = score.unsqueeze(-1)
            ...
            if torch.any(pending):
                ...
                if torch.any(ready):
                    ...
                    retry = ready & ~safe
                    if torch.any(retry):
                        retry_steps = int(getattr(self.cfg, "tactile_baseline_retry_steps", 2))
            ...
            valid_mask = ~self._tactile_ref_pending
            if not (has_tact_l or has_tact_r):
                valid_mask = torch.zeros_like(valid_mask, dtype=torch.bool, device=self.device)
            tactile_valid = ((score_l > diff_thresh) | (score_r > diff_thresh)) & valid_mask
            tactile_valid = tactile_valid.float().unsqueeze(-1)

        self.extras.setdefault("log", {})
        self.extras["log"]["tactile_valid_ratio"] = float(tactile_valid.mean().item())
        self.extras["log"]["tactile_score_mean"] = float(tactile_score.mean().item())
        if (
            getattr(self, "reward_print_interval", 0) > 0
            and self.step_count % self.reward_print_interval == 0
        ):
            env0_valid = bool(tactile_valid[0].item())
            env0_score = float(tactile_score[0].item())
            print(
                f"[触觉] step {self.step_count}: valid_ratio={tactile_valid.mean().item():.3f} "
                f"env0_valid={env0_valid} env0_score={env0_score:.4f}"
            )
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        super()._reset_idx(env_ids)
        if hasattr(self, "_tactile_ref_pending"):
            warmup_steps = int(getattr(self.cfg, "tactile_baseline_warmup_steps", 4))
