...
"""Cylinder grasping with a wrist camera and two inner tactile sensors."""
...
class CylinderGraspingVisionOnlyCfg(DirectRLEnvCfg):
    ...
    cylinder = RigidObjectCfg(
        ...
        spawn=sim_utils.CylinderCfg(
            radius=0.02,  # 圆柱体半径 2cm
            height=0.06,  # 圆柱体高度 6cm（体积提升 1.5x，保持半径不变）
    ...
    # two inner tactile sensors (left/right, depth-only)
    ...
    observation_space = {
        ...
        "tactile_left_depth_resnet": 64,
        "tactile_right_depth_resnet": 64,
    ...
    lift_weight = 15.0
    penalty_schedule_steps = 1000_000
    ...
    tactile_contact_weight = 5.0
    tactile_contact_threshold = 0.8
    ...
    cylinder_height = 0.06  # 匹配CylinderCfg中的高度（增大 1.5x）
    cylinder_radius = 0.02  # 匹配CylinderCfg中的半径
    lift_height = 0.06
    ground_height = 0.0  # 地面高度，用于判断机械臂关节是否碰撞地面

    # randomization
    cylinder_pos_range = 0.1  # plate half-width for random placement
...
class CylinderGraspingVisionOnlyEnv(DirectRLEnv):
    """Cylinder grasping environment with wrist vision and two inner tactile sensors."""
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        if dev_type is None:
            dev_type = "cuda" if (isinstance(dev, str) and str(dev).startswith("cuda")) else "cpu"
        ...
        if hasattr(self, "_use_resnet18") and self._use_resnet18:
            ...
            with torch.no_grad(), torch.amp.autocast(device_type="cuda", enabled=use_amp, dtype=torch.float16):
        ...
        tact_l_raw = self.gsmini_left.data.output.get("camera_depth") if hasattr(self, "gsmini_left") else None
        tact_r_raw = self.gsmini_right.data.output.get("camera_depth") if hasattr(self, "gsmini_right") else None
        target_h, target_w = getattr(self.cfg, "tactile_img_res_hw", (96, 128))

        def _prep_depth(depth_tensor):
            if depth_tensor is None:
                return torch.zeros((self.num_envs, target_h, target_w, 3), dtype=torch.float32, device=self.device)
            d = depth_tensor.to(device=self.device, dtype=torch.float32) / 255.0
            if d.shape[1] != target_h or d.shape[2] != target_w:
                dn = d.permute(0, 3, 1, 2).contiguous()
                dn = F.interpolate(dn, size=(target_h, target_w), mode="bilinear", align_corners=False)
                d = dn.permute(0, 2, 3, 1).contiguous()
            return d.repeat(1, 1, 1, 3)

        tact_l = _prep_depth(tact_l_raw)
        tact_r = _prep_depth(tact_r_raw)

        if hasattr(self, "_use_resnet18") and self._use_resnet18:
            xl = tact_l.permute(0, 3, 1, 2).contiguous()
            xr = tact_r.permute(0, 3, 1, 2).contiguous()
            xl = (xl - self._imgnet_mean) / self._imgnet_std
            xr = (xr - self._imgnet_mean) / self._imgnet_std
            with torch.no_grad(), torch.amp.autocast(device_type="cuda", enabled=use_amp, dtype=torch.float16):
                fl = self._resnet18(xl).view(self.num_envs, 512)[:, :64]
                fr = self._resnet18(xr).view(self.num_envs, 512)[:, :64]
        else:
            fl = torch.zeros((self.num_envs, 64), device=self.device, dtype=torch.float32)
            fr = torch.zeros((self.num_envs, 64), device=self.device, dtype=torch.float32)
    ...
    #     def _contact_ratio_from_depth(depth_tensor, thr: float):
    #         if depth_tensor is None:
    ...
    #             f"(w={self.cfg.success_reward_weight}), tactile={tactile_contact_reward.mean().item():.3f} "
    #             f"(w={self.cfg.tactile_contact_weight}), "
    ...
    def _get_rewards(self) -> torch.Tensor:
        """Rewards:
        - reach: bring ee close to object
        - tactile_any: encourage making contact (either finger) -> dense
        - tactile_bi: encourage bilateral contact -> used as lift gate
        - tactile_balance: encourage symmetric contact -> helps avoid single-side poke
        - lift (shaped): relative COM height (z - spawn_z), gated by bilateral contact
        - lift_v (shaped): upward velocity, gated by bilateral contact
        - success: height threshold aligned with done success
        - penalties: action rate + joint vel (+ optional tilt/spin penalties gated)
        """
        # --- states
        cylinder_pos = self._cylinder.data.root_pos_w
        cylinder_quat = self._cylinder.data.root_quat_w
        ...
        hand_pos = self._robot.data.body_link_pos_w[:, self._body_idx]
        hand_quat = self._robot.data.body_link_quat_w[:, self._body_idx]
        ee_pos, _ = math_utils.combine_frame_transforms(hand_pos, hand_quat, self._offset_pos, self._offset_rot)
        ...
        ratio_l = _contact_ratio_from_depth(tact_l, thr)  # ~0..1 (often very small)
        ratio_r = _contact_ratio_from_depth(tact_r, thr)

        # --- ratio -> learnable strength in [0,1]
        # r_scale: "how much pixel ratio counts as solid contact"
        # If your ratio is usually 0.001~0.01, start with 0.005.
        r_scale = float(getattr(self.cfg, "tactile_ratio_scale", 0.005))
        s_l = torch.tanh(ratio_l / (r_scale + 1e-6))  # 0..1
        s_r = torch.tanh(ratio_r / (r_scale + 1e-6))  # 0..1

        # --- tactile rewards
        tactile_any = 0.5 * (s_l + s_r)                 # dense: either side contact
        tactile_bi = torch.minimum(s_l, s_r)            # bilateral: short-board principle (easier than sqrt)
        tactile_balance = -torch.abs(s_l - s_r)         # [-1,0], encourage symmetry

        # --- lift gate (optional bias to avoid "gate never opens" early)
        lift_gate_bias = float(getattr(self.cfg, "lift_gate_bias", 0.0))  # try 0.1~0.2 if still too hard
        g = torch.clamp(tactile_bi + lift_gate_bias, 0.0, 1.0)

        # --- shaped lift (relative height)
        h_rel = cylinder_pos[:, 2] - self._cylinder_spawn_height_per_env
        h_norm = torch.clamp(h_rel / (self.cfg.lift_height + 1e-6), 0.0, 1.0)
        lift_reward = g * h_norm

        # --- shaped upward velocity (helps “抓住→往上”）
        vz_max = float(getattr(self.cfg, "lift_vz_max", 0.2))
        vz = cylinder_lin_vel[:, 2]
        lift_v = g * torch.clamp(vz, 0.0, vz_max) / (vz_max + 1e-6)

        # --- success aligned with done condition
        success_reward = (cylinder_pos[:, 2] > self._lift_target_height_per_env).float()

        # --- optional anti-cheat penalties (gated)
        roll, pitch, _ = euler_xyz_from_quat(cylinder_quat)
        tilt = torch.abs(roll) + torch.abs(pitch)
        tilt_pen = g * tilt
        spin_pen = g * torch.norm(cylinder_ang_vel, dim=-1)

        # --- penalties (keep your structure)
        action_curr = torch.where(torch.isfinite(self.processed_actions), self.processed_actions, torch.zeros_like(self.processed_actions))
        action_prev = torch.where(torch.isfinite(self.prev_actions), self.prev_actions, torch.zeros_like(self.prev_actions))
        action_diff = action_curr - action_prev
        action_rate_penalty = torch.sum(action_diff * action_diff, dim=-1)
        ...
        # schedule for penalty weights
        schedule_reached = self.step_count >= self.cfg.penalty_schedule_steps
        action_weight = self.cfg.action_rate_weight_final if schedule_reached else self.cfg.action_rate_weight_initial
        joint_weight = self.cfg.joint_vel_weight_final if schedule_reached else self.cfg.joint_vel_weight_initial

        # --- tactile weights (defaults; can override in cfg if you want)
        w_any = float(getattr(self.cfg, "tactile_any_weight", self.cfg.tactile_contact_weight))
        w_bi = float(getattr(self.cfg, "tactile_bi_weight", self.cfg.tactile_contact_weight))
        w_bal = float(getattr(self.cfg, "tactile_balance_weight", 1.0))

        # --- tilt/spin weights (negative)
        w_tilt = float(getattr(self.cfg, "tilt_penalty_weight", 2.0))
        w_spin = float(getattr(self.cfg, "spin_penalty_weight", 0.1))

        rewards = (
            self.cfg.reach_weight * reach_reward
            + w_any * tactile_any
            + w_bi * tactile_bi
            + w_bal * tactile_balance
            + self.cfg.lift_weight * lift_reward
            + 2.0 * lift_v
            + self.cfg.success_reward_weight * success_reward
            - w_tilt * tilt_pen
        ...
        if self.reward_print_interval > 0 and (self.step_count + 1) % self.reward_print_interval == 0:
            print(
                ...
                f"ratio_l={ratio_l.mean().item():.4f} ratio_r={ratio_r.mean().item():.4f} "
                f"t_any={tactile_any.mean().item():.3f}(w={w_any}) "
                f"t_bi={tactile_bi.mean().item():.3f}(w={w_bi}) "
                f"g={g.mean().item():.3f} "
                f"lift={lift_reward.mean().item():.3f}(w={self.cfg.lift_weight}) "
                f"lift_v={lift_v.mean().item():.3f}(x2.0) "
                f"success={success_reward.mean().item():.3f}(w={self.cfg.success_reward_weight}) "
                f"tilt_pen={tilt_pen.mean().item():.3f}(x-{w_tilt}) "
                f"spin_pen={spin_pen.mean().item():.3f}(x-{w_spin}) "
                f"act_pen={action_rate_penalty.mean().item():.6f}(w={action_weight}) "
                f"joint_pen={joint_vel_penalty.mean().item():.6f}(w={joint_weight}) "
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # When an env finishes an episode, compute its avg reward and update window
        ...
        # print windowed avg every reward_print_interval steps
