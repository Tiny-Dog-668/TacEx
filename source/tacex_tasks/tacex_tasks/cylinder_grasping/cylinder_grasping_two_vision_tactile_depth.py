...
class CylinderGraspingFourVisionTactileCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(
        ...
        lazy_sensor_update=False,  # 确保每步都更新传感器，避免读到未更新帧
    )
    ...
    # note: outer tactile sensors removed for two-vision variant
    ...
    observation_space = {
        ...
        # outer tactile features removed
...
class CylinderGraspingFourVisionTactileEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # outer sensors removed in this variant
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        # Tactile depth from GelSight sensors (left/right); use camera_depth in [0,255]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        def _contact_ratio_from_depth(depth_tensor, thr: float):
            ...
            # normalize to [0,1]; larger normalized depth -> further surface; 采用“白=接触”的判定
        ...
        if (
            ...
            # 额外打印：深度均值/最小/最大/中心像素 与 阈值/接触占比（用于调阈值）
            try:
                # 统计每个 env 的归一化深度统计
                def _depth_stats(depth_tensor):
                    ...
                    d = depth_tensor.to(device=self.device, dtype=torch.float32) / 255.0
                    dmean = d.mean(dim=(1, 2, 3), keepdim=False)
                    dmin = d.amin(dim=(1, 2, 3), keepdim=False)
                    dmax = d.amax(dim=(1, 2, 3), keepdim=False)
                    H, W = d.shape[1], d.shape[2]
                    dcenter = d[:, H // 2, W // 2, 0]
                    return dmean, dmin, dmax, dcenter

                dmean_l, dmin_l, dmax_l, dcenter_l = _depth_stats(tact_l)
                dmean_r, dmin_r, dmax_r, dcenter_r = _depth_stats(tact_r)
                print(
                    f"[触觉深度] step {self.step_count + 1}: "
                    f"dmean_l={dmean_l.mean().item():.4f}, dmean_r={dmean_r.mean().item():.4f}, "
                    f"dmin_l={dmin_l.mean().item():.4f}, dmax_l={dmax_l.mean().item():.4f}, center_l={dcenter_l.mean().item():.4f}, "
                    f"dmin_r={dmin_r.mean().item():.4f}, dmax_r={dmax_r.mean().item():.4f}, center_r={dcenter_r.mean().item():.4f}, "
