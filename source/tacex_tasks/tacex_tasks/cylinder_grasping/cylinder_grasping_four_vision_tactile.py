...
class CylinderGraspingFourVisionTactileCfg(DirectRLEnvCfg):
    ...
    # only use tactile RGB; drop marker motion simulation
    ...
    gsmini_left_out.data_types = ["tactile_rgb"]
    ...
    gsmini_right_out.data_types = ["tactile_rgb"]
    ...
    observation_space = {
        ...
        "tactile_left_out_resnet": 512,
        "tactile_right_out_resnet": 512,
...
class CylinderGraspingFourVisionTactileEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        # Tactile RGB from GelSight sensors (left/right + left_out/right_out); normalize to [0,1]
        tact_l_raw = self.gsmini_left.data.output.get("tactile_rgb")
        tact_r_raw = self.gsmini_right.data.output.get("tactile_rgb")
        tact_lo_raw = self.gsmini_left_out.data.output.get("tactile_rgb") if hasattr(self, "gsmini_left_out") else None
        tact_ro_raw = self.gsmini_right_out.data.output.get("tactile_rgb") if hasattr(self, "gsmini_right_out") else None
        ...
        if tact_lo_raw is None:
            tact_lo = torch.zeros((self.num_envs, target_h, target_w, 3), dtype=torch.float32, device=self.device)
        else:
            tact_lo = tact_lo_raw.to(device=self.device, dtype=torch.float32) / 255.0
            if tact_lo.shape[1] != target_h or tact_lo.shape[2] != target_w:
                xlo_rs = tact_lo.permute(0, 3, 1, 2).contiguous()
                xlo_rs = F.interpolate(xlo_rs, size=(target_h, target_w), mode="bilinear", align_corners=False)
                tact_lo = xlo_rs.permute(0, 2, 3, 1).contiguous()
        if tact_ro_raw is None:
            tact_ro = torch.zeros((self.num_envs, target_h, target_w, 3), dtype=torch.float32, device=self.device)
        else:
            tact_ro = tact_ro_raw.to(device=self.device, dtype=torch.float32) / 255.0
            if tact_ro.shape[1] != target_h or tact_ro.shape[2] != target_w:
                xro_rs = tact_ro.permute(0, 3, 1, 2).contiguous()
                xro_rs = F.interpolate(xro_rs, size=(target_h, target_w), mode="bilinear", align_corners=False)
                tact_ro = xro_rs.permute(0, 2, 3, 1).contiguous()
