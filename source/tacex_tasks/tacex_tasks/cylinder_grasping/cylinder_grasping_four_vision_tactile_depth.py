...
from isaaclab.sensors import Camera, CameraCfg, FrameTransformer, FrameTransformerCfg, TiledCamera, TiledCameraCfg
...
@configclass
class CylinderGraspingFourVisionTactileCfg(DirectRLEnvCfg):
    ...
    wrist_camera: TiledCameraCfg = TiledCameraCfg(
        ...
        # third-person camera (tiled; fixed to env root)
    ...
    # additional outer tactile sensors (suffix _out)
    ...
    gsmini_left_out.data_types = ["camera_depth"]
    ...
    gsmini_right_out.data_types = ["camera_depth"]
    ...
    observation_space = {
        ...
        # third-person camera features only
        "third_resnet": 512,
        "tactile_left_depth_resnet": 512,
        "tactile_right_depth_resnet": 512,
        "tactile_left_out_depth_resnet": 512,
        "tactile_right_out_depth_resnet": 512,
    ...
    tactile_contact_weight = 5.0  # weight for contact-area reward
    tactile_contact_threshold = 0.2  # depth_norm threshold in [0,1] to count a pixel as contact
...
class CylinderGraspingFourVisionTactileEnv(DirectRLEnv):
    """Cylinder grasping environment with privileged state and additional camera stream."""

    cfg: CylinderGraspingFourVisionTactileCfg

    def __init__(self, cfg: CylinderGraspingFourVisionTactileCfg, render_mode: str | None = None, **kwargs):
        ...
        # Optional ResNet18 backbone for wrist/third/tactile RGB -> 512-d feature
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        # Third-person camera observation
        third_rgb = None
        if hasattr(self, "third_person_camera") and self.third_person_camera is not None:
        ...
        # Tactile depth from GelSight sensors (left/right + left_out/right_out); use camera_depth in [0,255]
        tact_l_raw = self.gsmini_left.data.output.get("camera_depth")
        tact_r_raw = self.gsmini_right.data.output.get("camera_depth")
        tact_lo_raw = self.gsmini_left_out.data.output.get("camera_depth") if hasattr(self, "gsmini_left_out") else None
        tact_ro_raw = self.gsmini_right_out.data.output.get("camera_depth") if hasattr(self, "gsmini_right_out") else None
        has_tact_l = tact_l_raw is not None
        has_tact_r = tact_r_raw is not None
        has_tact_lo = tact_lo_raw is not None
        has_tact_ro = tact_ro_raw is not None
        target_h, target_w = getattr(self.cfg, "tactile_img_res_hw", (240, 320))
        def _prep_depth(depth_tensor):
            # expected NHWC with C=1; convert to float [0,1], resize if needed, then expand to 3 channels
            ...
            # expand single channel to 3 for ResNet
            return d.repeat(1, 1, 1, 3)

        tact_l = _prep_depth(tact_l_raw)
        tact_r = _prep_depth(tact_r_raw)
        tact_lo = _prep_depth(tact_lo_raw)
        tact_ro = _prep_depth(tact_ro_raw)
        ...
        if hasattr(self, "_use_resnet18") and self._use_resnet18:
            ...
            if hasattr(self, "_imagenet_mean"):
                xw = (xw - self._imagenet_mean) / self._imagenet_std
        ...
        if hasattr(self, "_use_resnet18") and self._use_resnet18:
            ...
            xlo = tact_lo.permute(0, 3, 1, 2).contiguous().to(self.device)
            xro = tact_ro.permute(0, 3, 1, 2).contiguous().to(self.device)
            if hasattr(self, "_imagenet_mean"):
                xl = (xl - self._imagenet_mean) / self._imagenet_std
                xr = (xr - self._imagenet_mean) / self._imagenet_std
                xlo = (xlo - self._imagenet_mean) / self._imagenet_std
                xro = (xro - self._imagenet_mean) / self._imagenet_std
            with torch.no_grad(), autocast(enabled=use_amp, dtype=torch.float16):
                fl = self._resnet18(xl).view(self.num_envs, 512)
                fr = self._resnet18(xr).view(self.num_envs, 512)
                flo = self._resnet18(xlo).view(self.num_envs, 512)
                fro = self._resnet18(xro).view(self.num_envs, 512)
        else:
            fl = torch.zeros((self.num_envs, 512), device=self.device)
            fr = torch.zeros((self.num_envs, 512), device=self.device)
            flo = torch.zeros((self.num_envs, 512), device=self.device)
            fro = torch.zeros((self.num_envs, 512), device=self.device)
        ...
        obs = {
            ...
            "tactile_left_out_depth_resnet": flo,
            "tactile_right_out_depth_resnet": fro,
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # tactile contact reward (inner tactile sensors: left/right, depth in [0,255])
        ...
        def _contact_ratio_from_depth(depth_tensor, thr: float):
            ...
            # normalize to [0,1]; in this pipeline larger normalized depth -> further surface (contact tends to increase depth)
            d = depth_tensor.to(device=self.device, dtype=torch.float32) / 255.0  # [N,H,W,1]
            contact_px = (d < thr).float()
            ratio = contact_px.mean(dim=(1, 2, 3), keepdim=False)
            return ratio
        ...
        # encourage any contact area (either pad): average of contact pixel ratios
        ...
        if (
            ...
            # 额外打印：深度均值与阈值/接触占比（用于调阈值）
            try:
                # 统计每个 env 的归一化深度均值
                def _depth_mean(depth_tensor):
                    if depth_tensor is None:
                        return torch.zeros((self.num_envs,), device=self.device)
                    d = depth_tensor.to(device=self.device, dtype=torch.float32) / 255.0
                    return d.mean(dim=(1, 2, 3), keepdim=False)

                dmean_l = _depth_mean(tact_l)
                dmean_r = _depth_mean(tact_r)
                print(
                    f"[触觉深度] step {self.step_count + 1}: "
                    f"dmean_l={dmean_l.mean().item():.4f}, dmean_r={dmean_r.mean().item():.4f}, "
                    f"thr={thr:.3f}, ratio_l={ratio_l.mean().item():.3f}, ratio_r={ratio_r.mean().item():.3f}"
                )
            except Exception:
                pass
