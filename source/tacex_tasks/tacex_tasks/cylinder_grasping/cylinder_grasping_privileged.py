...
"""Cylinder Grasping Environment with Privileged Information (No Vision, No Tactile)."""
...
@configclass
class CylinderGraspingPrivilegedCfg(DirectRLEnvCfg):
    """Configuration for the cylinder grasping environment with privileged information."""
    ...
    observation_space = {
        ...
        "cylinder_pos": 3,  # cylinder position
        "cylinder_quat": 4,  # cylinder orientation
        "cylinder_lin_vel": 3,  # cylinder linear velocity
        "cylinder_ang_vel": 3,  # cylinder angular velocity
        "gripper_pos": 3,  # gripper position
        "gripper_quat": 4,  # gripper orientation
        "gripper_lin_vel": 3,  # gripper linear velocity
        "gripper_ang_vel": 3,  # gripper angular velocity
        "target_pos": 3,  # target relative position
        "target_distance": 1,  # target distance
    ...
    cylinder_height = 0.04  # 匹配CylinderCfg中的高度
...
class CylinderGraspingPrivilegedEnv(DirectRLEnv):
    """Cylinder grasping environment with privileged information."""

    cfg: CylinderGraspingPrivilegedCfg

    def __init__(self, cfg: CylinderGraspingPrivilegedCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)

        # Older checkpoint configs may miss fields added after training. Hydrate
        # them from the current task defaults so play remains backward compatible.
        for attr_name in (
            "reach_sigma",
            "reach_weight",
            "minimal_lift_height",
            "lift_weight",
            "penalty_schedule_steps",
            "action_rate_weight_initial",
            "action_rate_weight_final",
            "joint_vel_weight_initial",
            "joint_vel_weight_final",
            "success_height",
            "success_reward_weight",
            "safety_plate_clearance",
            "finger_tip_offset",
            "plate_top_offset",
            "cylinder_height",
            "cylinder_radius",
            ...
            "cylinder_pos_range",
            "cylinder_rot_range",
            "robot_joint_pos_noise",
            "robot_joint_vel_noise",
            "action_noise_scale",
            "reset_jitter_max_steps",
            "reward_print_interval",
        ):
            if not hasattr(self.cfg, attr_name):
                setattr(self.cfg, attr_name, getattr(CylinderGraspingPrivilegedCfg, attr_name))
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        obs = {
            ...
            "cylinder_lin_vel": cylinder_lin_vel,
            "cylinder_ang_vel": cylinder_ang_vel,
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cylinder_pos = self._cylinder.data.root_pos_w
        ee_pos = self._robot.data.body_link_pos_w[:, self._body_idx]
        ...
        # action_rate penalty: sum_i (a_t,i - a_{t-1,i})^2
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        if max_jitter_steps > 0:
            ...
            jitter = torch.zeros(len(env_ids), dtype=self.episode_length_buf.dtype, device=self.device)
