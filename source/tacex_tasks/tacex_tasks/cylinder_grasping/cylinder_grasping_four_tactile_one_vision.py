...
"""Cylinder Grasping Environment with wrist camera and four tactile sensors (inner + downward).

Changes in this variant:
- Remove the third-person camera.
- Keep inner left/right GelSight Mini sensors and downward left/right sensors (no outer pads).
"""
...
@configclass
class CylinderGraspingFourTactileOneVisionCfg(DirectRLEnvCfg):
    """Configuration for the cylinder grasping environment with wrist-camera vision and four tactile sensors."""
    ...
    observation_space = {
        ...
        # no third-person camera in this variant
    ...
    lift_weight = 15.0
    penalty_schedule_steps = 500_000
    action_rate_weight_initial = -1e-4
    action_rate_weight_final = -1e-1
    joint_vel_weight_initial = -1e-4
    joint_vel_weight_final = -1e-1
    # 是否对惩罚项做退火（默认关闭：使用 initial 常量权重）
    use_penalty_annealing = False
    # 成功判定与成功奖励阈值（10 cm）
    success_height = 0.10
    success_reward_weight = 100.0
    # tactile contact reward (separate inner vs downward sensors)
    tactile_contact_weight_inner = 5.0   # weight for inner pads (left/right)
    tactile_contact_weight_down = 1.0    # weight for downward pads (left_down/right_down)
    tactile_contact_threshold = 0.9  # depth_norm threshold in [0,1] to count a pixel as contact
...
class CylinderGraspingFourTactileOneVisionEnv(DirectRLEnv):
    """Cylinder grasping environment with privileged state, one wrist camera and four tactile sensors."""

    cfg: CylinderGraspingFourTactileOneVisionCfg

    def __init__(self, cfg: CylinderGraspingFourTactileOneVisionCfg, render_mode: str | None = None, **kwargs):
        ...
        # wrist camera offset is defined in cfg; no extra buffers
        ...
        # Lightweight 1-channel tactile encoder shared for four tactile streams -> 256-D
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # keep previous actions for rate penalties
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        # Wrist camera world position (not used)
        ...
        # 触觉特征：使用共享 1 通道轻量编码器，输出 256 维
        with torch.no_grad(), torch.amp.autocast(device_type=dev_type, enabled=use_amp, dtype=torch.float16):
            fl = self._tactile_encoder(tact_l).view(self.num_envs, 256)
            fr = self._tactile_encoder(tact_r).view(self.num_envs, 256)
            fld = self._tactile_encoder(tact_ld).view(self.num_envs, 256)
            frd = self._tactile_encoder(tact_rd).view(self.num_envs, 256)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # penalty weights (no annealing by default)
        if getattr(self.cfg, "use_penalty_annealing", False):
            ...
            action_weight = self.cfg.action_rate_weight_initial
            joint_weight = self.cfg.joint_vel_weight_initial
        ...
        tact_ld = self.gsmini_left_down.data.output.get("camera_depth") if hasattr(self, "gsmini_left_down") else None
        tact_rd = self.gsmini_right_down.data.output.get("camera_depth") if hasattr(self, "gsmini_right_down") else None
        ...
        ratio_ld = _contact_ratio_from_depth(tact_ld, thr)
        ratio_rd = _contact_ratio_from_depth(tact_rd, thr)
        # encourage any contact area for each group (inner vs down)
        tactile_contact_inner = 0.5 * (ratio_l + ratio_r)
        tactile_contact_down = 0.5 * (ratio_ld + ratio_rd)

        rewards = (
            self.cfg.reach_weight * reach_reward
            + self.cfg.lift_weight * lift_reward
            + self.cfg.success_reward_weight * success_reward
            + self.cfg.tactile_contact_weight_inner * tactile_contact_inner
            + self.cfg.tactile_contact_weight_down * tactile_contact_down
        ...
        if self.cfg.reward_print_interval > 0 and ((self.step_count + 1) % self.cfg.reward_print_interval == 0):
            try:
                print(
                    ...
                    f"tactile_inner={tactile_contact_inner.mean().item():.3f} (w={self.cfg.tactile_contact_weight_inner}), "
                    f"tactile_down={tactile_contact_down.mean().item():.3f} (w={self.cfg.tactile_contact_weight_down}), "
                    f"action_penalty={action_rate_penalty.mean().item():.6f} (w={action_weight}), "
                    f"joint_penalty={joint_vel_penalty.mean().item():.6f} (w={joint_weight}), "
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # When an env finishes an episode, print its average reward and reset trackers
        done_ids = dones.nonzero(as_tuple=False).squeeze(-1)
        if done_ids.numel() > 0:
            # per-env average for finished envs in this step; accumulate into window
            ...
            # optional per-env details (disabled by default)
