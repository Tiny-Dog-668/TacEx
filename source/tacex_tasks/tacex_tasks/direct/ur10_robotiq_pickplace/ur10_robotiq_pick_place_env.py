...
GRIPPER_COUPLING_RULES = (
    ...
    ("left_outer_finger_joint", 0.0, 0.0),
    ("right_outer_finger_joint", 0.0, 0.0),
    ("left_inner_finger_joint", -1.0, 0.0),
    ("right_inner_finger_joint", -1.0, 0.0),
    ("left_inner_finger_pad_joint", 1.0, 0.0),
    ("right_inner_finger_pad_joint", 1.0, 0.0),
)
...
GRIP_BODY_FALLBACKS = ("robotiq_base_link", "ee_link", "wrist_3_link")
...
# Use the validated merged UR10+Robotiq asset by default. The previous topic_based asset introduces nested
# articulation roots and fails Articulation initialization.
DEFAULT_UR10_ROBOTIQ_USD_PATH = str(_UR_ROBOTIQ_ASSET_DIR / "ur10_robotiq_140.usda")
UR10_ROBOTIQ_USD_PATH = os.environ.get("UR10_ROBOTIQ_USD_PATH", DEFAULT_UR10_ROBOTIQ_USD_PATH)
...
@configclass
class UR10RobotiqPickPlaceEnvCfg(DirectRLEnvCfg):
    episode_length_s = 3.33
    decimation = 2
    action_space = 4
    observation_space = 27
    state_space = 0
    robot_usd_path = UR10_ROBOTIQ_USD_PATH
    ...
    grip_center_world_offset = (0.0, 0.0, 0.0)
    ...
    ee_action_noise_std = 0.0
    gripper_action_noise_std = 0.0
    ...
    table_pos = (1.0, 0.0, 0.02)
    object_spawn_center = (1.0, 0.0, 0.0)
    object_xy_noise = 0.1
    lift_height = 0.08
    reach_radius = 0.08
    fall_height = -0.02
...
class UR10RobotiqPickPlaceEnv(DirectRLEnv):
    cfg: UR10RobotiqPickPlaceEnvCfg

    def __init__(self, cfg: UR10RobotiqPickPlaceEnvCfg, render_mode: str | None = None, **kwargs):
