"""Tactile-cross-alpha policy with trainable tactile CNN reconstruction."""
...
class OccludedGraspingVTTactileCrossAlphaReconPolicy(GaussianMixin, Model):
    """Compute tactile features in-policy, reconstruct tactile RGB, then alpha-mix T and T'."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.tactile_token_proj = nn.ModuleList(
            [nn.Sequential(nn.LayerNorm(self.tactile_feature_dim), nn.Linear(self.tactile_feature_dim, self.fused_dim), act_cls())
             for _ in self.tactile_raw_keys]
        )
        self.tactile_sensor_embed = nn.Embedding(len(self.tactile_raw_keys), self.fused_dim)
        ...
        self.tactile_cross_proj = nn.Sequential(
            nn.LayerNorm(len(self.tactile_raw_keys) * self.fused_dim),
            nn.Linear(len(self.tactile_raw_keys) * self.fused_dim, self.fused_dim),
            act_cls(),
        )
    ...
    def compute(self, inputs: Dict[str, Any], role: str = ""):
        ...
        tactile_batch = tactile_images.reshape(
            batch_size * len(self.tactile_raw_keys), 3, self.tactile_img_h, self.tactile_img_w
        )
        tactile_features_flat = self.tactile_encoder(tactile_batch)
        recon_images = self.tactile_decoder(tactile_features_flat)
        ...
        tactile_features = tactile_features_flat.reshape(
            batch_size, len(self.tactile_raw_keys), self.tactile_feature_dim
        )
        tactile_flat = tactile_features.reshape(batch_size, -1)
        tactile = self.tactile_base_proj(tactile_flat)

        tactile_tokens = torch.stack(
            [proj(tactile_features[:, idx]) for idx, proj in enumerate(self.tactile_token_proj)],
            dim=1,
        )
        sensor_ids = torch.arange(len(self.tactile_raw_keys), device=self.device)
        ...
        recon_target = tactile_images.reshape(
            batch_size * len(self.tactile_raw_keys), 3, self.tactile_img_h, self.tactile_img_w
        )
...
def quick_tactile_cross_alpha_recon_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    model = OccludedGraspingVTTactileCrossAlphaReconPolicy(obs_space, action_space, device=device)
    states = {
    ...
    print(
        f"[tactile_cross_alpha_recon_smoke_test] mean shape={tuple(mean.shape)} "
