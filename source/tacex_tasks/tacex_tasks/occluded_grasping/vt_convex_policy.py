"""Lightweight convex-combination VT policy for occluded grasping."""
...
class OccludedGraspingVTConvexPolicy(GaussianMixin, Model):
    """Late-fusion policy with scalar alpha convex-combining vision and tactile summaries."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for convex VT policy")
...
def quick_convex_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    model = OccludedGraspingVTConvexPolicy(
    ...
    print(f"[convex_smoke_test] mean shape={tuple(mean.shape)} grad_norm={grad_norm:.6f}")
