"""Temporal strong-gate VT policy with auxiliary task heads."""
...
class OccludedGraspingVTTemporalStrongGatePolicy(GaussianMixin, Model):
    """Direct action policy with binary tactile gates, temporal compression, and auxiliary heads."""
    ...
    def __init__(
        ...
        temporal_hidden_layers: Iterable[int] | None = None,
        ...
        **kwargs: Any,
        ...
        if tactile_keys is None:
            tactile_keys = (
                "tactile_left_rgb_resnet",
                "tactile_right_rgb_resnet",
                "tactile_left_down_rgb_resnet",
                "tactile_right_down_rgb_resnet",
            )
        if mlp_layers is None:
            mlp_layers = (512, 256, 128, 64)
        if aux_hidden_layers is None:
            aux_hidden_layers = (256, 128)
        if temporal_hidden_layers is None:
            temporal_hidden_layers = (512,)
        if pair_hidden_layers is None:
            pair_hidden_layers = (256,)
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for temporal strong-gate policy")

        required_keys = [self.vision_key, self.proprio_key, self.aux_input_key, self.prev_action_key]
        ...
        for key in (
            self.aux_reliability_gt_key,
            self.aux_stage_gt_key,
            self.aux_stage_onehot_gt_key,
            self.aux_probe_gt_key,
            self.aux_grasp_gt_key,
            self.bottom_valid_hist_key,
            self.inner_valid_hist_key,
        ):
        ...
        tactile_dim = int(self._obs_dims[self.tactile_keys[0]])
        self.temporal_out_dim = int(temporal_out_dim)
        if self.temporal_out_dim <= 0:
            raise ValueError(f"temporal_out_dim must be positive, got {self.temporal_out_dim}")

        self.temporal_compressor = self._build_mlp(
            input_dim=tactile_dim,
            hidden_layers=temporal_hidden_layers,
            output_dim=self.temporal_out_dim,
            act_cls=act_cls,
        )
        self.probe_pair_mlp = self._build_mlp(
            input_dim=2 * self.temporal_out_dim,
            hidden_layers=pair_hidden_layers,
            output_dim=self.temporal_out_dim,
            act_cls=act_cls,
        )
        self.grasp_pair_mlp = self._build_mlp(
            input_dim=2 * self.temporal_out_dim,
            hidden_layers=pair_hidden_layers,
            output_dim=self.temporal_out_dim,
            act_cls=act_cls,
        ...
        action_in_dim = (
            int(self._obs_dims[self.vision_key])
            + self.temporal_out_dim
            + self.temporal_out_dim
            + int(self._obs_dims[self.proprio_key])
            + int(self._obs_dims[self.prev_action_key])
        )
        ...
        aux_in_dim = int(self._obs_dims[self.aux_input_key])
        ...
        self.probe_head = self._build_mlp(
            input_dim=self.temporal_out_dim,
            hidden_layers=(128,),
            output_dim=4,
            act_cls=act_cls,
        )
    ...
    def _compress_tactile(self, tactile_feats: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        return {key: self.temporal_compressor(feat) for key, feat in tactile_feats.items()}
    ...
    def _get_recent_tactile_masks(
        self, obs: Dict[str, Any], batch_size: int
        ...
        probe_gate = _mask_from_hist(self.bottom_valid_hist_key)
        grasp_gate = _mask_from_hist(self.inner_valid_hist_key)
        tactile_gate = torch.maximum(probe_gate, grasp_gate)
        return probe_gate, grasp_gate, tactile_gate
    ...
    def compute(self, inputs: Dict[str, Any], role: str = ""):
        ...
        compressed = self._compress_tactile(tactile_feats)

        probe_pair = self.probe_pair_mlp(
            torch.cat(
                [
                    compressed[self._tactile_probe_keys[0]],
                    compressed[self._tactile_probe_keys[1]],
                ],
                dim=-1,
            )
        )
        grasp_pair = self.grasp_pair_mlp(
            torch.cat(
                [
                    compressed[self._tactile_grasp_keys[0]],
                    compressed[self._tactile_grasp_keys[1]],
                ],
                dim=-1,
            )
        )

        probe_gate, grasp_gate, tactile_gate = self._get_recent_tactile_masks(obs, batch_size)
        h_probe = probe_gate * probe_pair
        h_grasp = grasp_gate * grasp_pair

        action_input = torch.cat([vision, h_probe, h_grasp, proprio, prev_action], dim=-1)
        ...
        stage_prob_gate = F.softmax(stage_logits_pred, dim=-1) if stage_logits_pred is not None else stage_prob
        probe_pred = torch.sigmoid(self.probe_head(probe_pair))
        grasp_pred = torch.sigmoid(self.grasp_head(grasp_pair))
