"""Vision-query tactile cross-attention policy with proprio-conditioned alpha mixing."""
...
class OccludedGraspingVTVisualCrossAlphaPolicy(GaussianMixin, Model):
    """Use visual query attention over four tactile tokens, then alpha-mix V and V'."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for visual-cross-alpha policy")
        ...
        self.visual_tactile_attn = nn.MultiheadAttention(
    ...
    def compute(self, inputs: Dict[str, Any], role: str = ""):
        ...
        fused = torch.cat([vision_mixed, *vector_features], dim=-1) if vector_features else vision_mixed
...
def quick_visual_cross_alpha_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    model = OccludedGraspingVTVisualCrossAlphaPolicy(
    ...
    loss.backward()
    print(f"[visual_cross_alpha_smoke_test] mean shape={tuple(mean.shape)}")
