"""Alpha policy with supervised task heads for occlusion and tactile reconstruction."""
...
class OccludedGraspingVTAlphaReconPolicy(GaussianMixin, Model):
    """VT alpha-gated policy with GT-supervised auxiliary task heads."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.tactile_dim = self.inner_dim + self.down_dim
        ...
        self.tactile_proj = nn.Sequential(
            ...
            nn.Linear(self.tactile_dim, self.fused_dim),
            act_cls(),
        )
        ...
        alpha_in_dim = 1 + 2 + 3 + 3 + self.proprio_dim
        self.alpha_gate = nn.Sequential(
            nn.Linear(alpha_in_dim, int(alpha_hidden_dim)),
            act_cls(),
            nn.Linear(int(alpha_hidden_dim), 1),
            nn.Sigmoid(),
        )

        mlp_input_dim = (2 * self.fused_dim) + self.proprio_dim
        mlp: list[nn.Module] = []
    ...
    def compute(self, inputs: Dict[str, Any], role: str = ""):
        ...
        tactile_raw = torch.cat([inner_raw, down_raw], dim=-1)
        ...
        alpha_in = torch.cat(
            [occlusion_pred, down_contact_pred, object_side_prob, inner_contact_pred, proprio],
            dim=-1,
        )
        alpha = self.alpha_gate(alpha_in)
        set_latest_alpha(alpha)

        vision = self.vision_proj(vision_raw)
        tactile = self.tactile_proj(tactile_raw)
        gated = torch.cat([(1.0 - alpha) * vision, alpha * tactile, proprio], dim=-1)

        hidden = self.mlp(gated)
        mu = self.mu(hidden)
