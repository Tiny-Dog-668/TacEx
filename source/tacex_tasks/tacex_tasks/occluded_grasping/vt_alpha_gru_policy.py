"""Alpha-gated VT policy with per-tactile GRU temporal encoders."""
...
class OccludedGraspingVTAlphaGRUPolicy(GaussianMixin, Model):
    """Late-fusion alpha policy using 2-layer GRU tactile encoders."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for alpha-GRU policy")
        ...
        for key in self.tactile_keys:
            ...
            if total_dim % self.tactile_time_window == 0:
                ...
                # Some registry paths expose the single-frame dim even though the Alpha-GRU env
                # stacks tactile features at runtime. Keep initialization compatible with both.
