"""Tactile-query visual cross-attention policy without alpha mixing."""
...
class OccludedGraspingVTTactileCrossPolicy(GaussianMixin, Model):
    """Use four tactile queries over visual context, then fixed residual fusion T + T'."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for tactile-cross policy")
    ...
    def compute(self, inputs: Dict[str, Any], role: str = ""):
        ...
        tactile_fused = tactile + tactile_cross
        vector_features = [self._flatten_obs(obs, key, batch_size) for key in self.vector_keys]
        fused = torch.cat([tactile_fused, *vector_features], dim=-1) if vector_features else tactile_fused
...
def quick_tactile_cross_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    model = OccludedGraspingVTTactileCrossPolicy(
        observation_space=obs_space,
        action_space=action_space,
        device=device,
    )
    states = {key: torch.rand(batch_size, 256) for key in obs_space.spaces if key != "proprio_obs"}
    states["proprio_obs"] = torch.rand(batch_size, 18)
    mean, log_std, _ = model.compute({"states": states}, role="policy")
    loss = mean.mean() + log_std.mean()
    loss.backward()
    print(f"[tactile_cross_smoke_test] mean shape={tuple(mean.shape)}")
