"""Lightweight alpha-gated vision-tactile policy for occluded grasping."""
...
class OccludedGraspingVTAlphaPolicy(GaussianMixin, Model):
    """Late-fusion policy with a proprio-conditioned scalar alpha gate."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_keys:
            raise ValueError("No tactile observation keys found for alpha-gated policy")
        ...
        self.tactile_proj = nn.Sequential(
            ...
            nn.Linear(tactile_dim, self.fused_dim),
            nn.ELU(),
        )
...
def quick_alpha_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    obs_space = gym.spaces.Dict(
        {
            ...
            "tactile_left_depth_resnet": gym.spaces.Box(-1.0, 1.0, shape=(128,), dtype=float),
            "tactile_right_depth_resnet": gym.spaces.Box(-1.0, 1.0, shape=(128,), dtype=float),
            "tactile_left_down_depth_resnet": gym.spaces.Box(-1.0, 1.0, shape=(128,), dtype=float),
            "tactile_right_down_depth_resnet": gym.spaces.Box(-1.0, 1.0, shape=(128,), dtype=float),
            "proprio_obs": gym.spaces.Box(-1.0, 1.0, shape=(18,), dtype=float),
        }
    )
    action_space = gym.spaces.Box(-1.0, 1.0, shape=(5,), dtype=float)

    model = OccludedGraspingVTAlphaPolicy(
    ...
    states = {
        ...
        "tactile_left_depth_resnet": torch.rand(batch_size, 128),
        "tactile_right_depth_resnet": torch.rand(batch_size, 128),
        "tactile_left_down_depth_resnet": torch.rand(batch_size, 128),
        "tactile_right_down_depth_resnet": torch.rand(batch_size, 128),
    ...
    print(f"[alpha_smoke_test] mean shape={tuple(mean.shape)} grad_norm={grad_norm:.6f}")
