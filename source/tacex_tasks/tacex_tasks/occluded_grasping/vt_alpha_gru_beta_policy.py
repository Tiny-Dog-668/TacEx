"""Alpha/beta-gated VT policy with per-tactile GRU temporal encoders."""
...
class OccludedGraspingVTAlphaGRUBetaPolicy(OccludedGraspingVTAlphaGRUPolicy):
    """Alpha-GRU policy with separate down/inner tactile beta gates."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.tactile_inner_keys:
            raise ValueError("No inner tactile observation keys found for alpha-GRU-beta policy")
        if not self.tactile_down_keys:
            raise ValueError("No down tactile observation keys found for alpha-GRU-beta policy")
...
def quick_alpha_gru_beta_policy_smoke_test(batch_size: int = 4, device: str = "cpu") -> None:
    ...
    model = OccludedGraspingVTAlphaGRUBetaPolicy(
    ...
    print(f"[alpha_gru_beta_smoke_test] mean shape={tuple(mean.shape)} grad_norm={grad_norm:.6f}")
