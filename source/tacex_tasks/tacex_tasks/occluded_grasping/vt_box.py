...
from .policy_alpha import get_latest_alpha_beta_means, get_latest_gate_mean, get_latest_gate_values
from .policy_aux_heads import (
    get_latest_alpha,
    get_latest_m_grasp,
    get_latest_m_probe,
    get_latest_w_grasp,
...
# Rectangle defined by the latest validated object motion bounds from the interactive drawer demo.
CAN_RANDOM_X_MIN = 0.5395
CAN_RANDOM_X_MAX = 0.6212
CAN_RANDOM_Y_MIN = -0.1192
CAN_RANDOM_Y_MAX = 0.1326
...
class OccludedGraspingVisionFourTactileBoxCfg(DirectRLEnvCfg):
    ...
    tactile_dino_total_frames = 2  # current + recent 5 frames -> 5 adjacent frame-pairs
...
class OccludedGraspingVisionFourTactileBoxEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: OccludedGraspingVisionFourTactileBoxCfg, render_mode: str | None = None, **kwargs):
        ...
        # Optional ResNet18 backbone for third-person RGB -> 256-d feature
        ...
        if self._use_resnet18:
            ...
            # Remove final fc and reduce the 512-d ResNet vector to the common 256-d camera feature.
            self._resnet18 = torch.nn.Sequential(
        ...
        self._ep_success_accum_sum = torch.tensor(0.0, device=self.device)
        self._ep_success_accum_count = torch.tensor(0, dtype=torch.long, device=self.device)
        ...
        self._drop_after_success_penalized_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
    ...
    def _spawn_drawer_scene_geometry(self) -> None:
        """Spawn the support floor and optional drawer/cabinet occluder geometry."""
        self._box_floor = RigidObject(self.cfg.box_floor)
        self.scene.rigid_objects["box_floor"] = self._box_floor

        if bool(getattr(self.cfg, "include_drawer_walls", True)):
        ...
        if not bool(getattr(self.cfg, "include_outer_cabinet_panels", True)):
            return
    ...
    def _initialize_can_positions(self):
        ...
        can_state[:, 2] = float(getattr(self.cfg, "can_reset_root_z", CAN_RESET_ROOT_Z))
    ...
    def _get_observations(self) -> dict[str, dict[str, torch.Tensor]]:
        ...
        self._latest_tactile_rgb_01 = tactile_curr
        ...
        if self._tactile_encoder_type == "dino":
            ...
            # Shared tactile encoder: batch all four sensors into one forward pass.
            tactile_batch = torch.cat([tact_l, tact_r, tact_ld, tact_rd], dim=0)
            with torch.no_grad(), torch.amp.autocast(device_type=dev_type, enabled=use_amp, dtype=torch.float16):
                encoded = self._tactile_encoder(tactile_batch).view(
                    len(self._tactile_sensor_keys), self.num_envs, self._tactile_feature_dim
                )
            fl, fr, fld, frd = encoded[0], encoded[1], encoded[2], encoded[3]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        drop_after_success = (
            self._success_achieved_buf
            & ~success_now
            & ~self._drop_after_success_penalized_buf
        )
        self._drop_after_success_penalized_buf |= drop_after_success
        drop_penalty = drop_after_success.float()
        ...
        for key in (
            ...
            "info/policy_alpha_mean",
            "info/policy_beta_mean",
        ...
        alpha_beta_means = get_latest_alpha_beta_means()
        if alpha_beta_means is not None:
            alpha_mean, beta_mean = alpha_beta_means
            log["info/policy_alpha_mean"] = torch.tensor(alpha_mean, device=self.device)
            log["info/policy_beta_mean"] = torch.tensor(beta_mean, device=self.device)
        ...
        if self.cfg.reward_print_interval > 0 and ((self.step_count + 1) % self.cfg.reward_print_interval == 0):
            ...
            try:
                ...
                if alpha_beta_means is not None:
                    alpha_mean, beta_mean = alpha_beta_means
                    gate_text += f", alpha_mean={alpha_mean:.4f}, beta_mean={beta_mean:.4f}"
                aux_gate_text = ""
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if done_ids.numel() > 0:
            ...
            ep_success = success[done_ids].to(dtype=torch.float32)
            self._ep_avg_accum_sum = self._ep_avg_accum_sum + ep_avg.sum()
            self._ep_avg_accum_count = self._ep_avg_accum_count + ep_avg.numel()
            self._ep_success_accum_sum = self._ep_success_accum_sum + ep_success.sum()
            self._ep_success_accum_count = self._ep_success_accum_count + ep_success.numel()
        ...
        log.pop("info/episode_success_rate_window", None)
        ...
        window_success_count = int(self._ep_success_accum_count.item())
        if window_success_count > 0:
            log["info/episode_success_rate_window"] = (
                self._ep_success_accum_sum / self._ep_success_accum_count.to(torch.float32)
            ).detach()
        ...
        if self.cfg.reward_print_interval > 0 and (self.step_count % self.cfg.reward_print_interval == 0):
            ...
            if cnt > 0:
                ...
                window_successes = float(self._ep_success_accum_sum.item())
                window_success_rate = float(self._ep_success_accum_sum.item()) / max(
                    int(self._ep_success_accum_count.item()), 1
                )
                print(
                    f"[Episode Avg Reward] avg={avg:.3f}, "
                    f"window_success_rate={window_success_rate:.3f}, "
                    f"episodes={cnt}, successes={window_successes:.0f}"
                )
                self._ep_avg_accum_sum = torch.tensor(0.0, device=self.device)
                self._ep_avg_accum_count = torch.tensor(0, dtype=torch.long, device=self.device)
                self._ep_success_accum_sum = torch.tensor(0.0, device=self.device)
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self._drop_after_success_penalized_buf[env_ids] = False
...
@configclass
class OccludedGraspingVisionFourTactileSelfOcclusionBoxCfg(OccludedGraspingVisionFourTactileBoxCfg):
    """Drawer task without scene-side occluders; only robot self-occlusion remains."""
