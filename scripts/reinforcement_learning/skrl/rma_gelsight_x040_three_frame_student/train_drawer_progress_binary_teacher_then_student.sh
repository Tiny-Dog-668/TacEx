...
# Train the Pulled-Drawer Progress Teacher, then distill its binary-tactile Student.
...
usage() {
    cat <<'EOF'
Usage: train_drawer_progress_binary_teacher_then_student.sh [options]
...
echo "[pipeline] Starting Progress Teacher with ${TEACHER_NUM_ENVS} envs"
run_isaac_python \
...
echo "[pipeline] Starting binary-tactile Student with ${STUDENT_NUM_ENVS} envs"
run_isaac_python \
...
echo "[pipeline] Progress Teacher and binary-tactile Student completed successfully"
