"""Export a paired Pulled-Drawer three-frame Student to TorchScript."""
...
def main() -> None:
    ...
    output = (
        ...
        if args.output
        else checkpoint.parent / "exported" / f"gelsight_pulled_drawer_{checkpoint.stem}.pt"
    )
    ...
    with torch.inference_mode():
        for batch in (1, 8):
            ...
            if max(errors) > 1.0e-5 or not all(
                torch.isfinite(value).all() for value in actual
            ):
                raise RuntimeError(
                    f"TorchScript validation failed for batch={batch}: {errors}"
                )
