...
class CylinderGraspingDemo(DirectRLEnv):
    ...
    def __init__(self, cfg: CylinderGraspingDemoCfg, render_mode: str | None = None, **kwargs):
        ...
        # We want the gripper to stay "vertical down": roll=pi, pitch=0, yaw is allowed to change.
        # In base frame (panda_link0), this roughly matches rpy ≈ [180°, 0°, yaw].
        self._ee_down_roll = float(np.pi)
        self._ee_down_pitch = 0.0
        # Maximum rotation (rad) we allow the controller to command per sim step (for stability)
        self._max_rot_step = 0.25
        # Gain applied to rotation vector command
        self._rot_gain = 1.0
    ...
    def _quat_wxyz_to_rpy_rad(quat_wxyz: torch.Tensor) -> torch.Tensor:
        """Quaternion (wxyz) -> roll-pitch-yaw (xyz), radians."""
    ...
    def _quat_wxyz_from_rpy_rad(roll: torch.Tensor, pitch: torch.Tensor, yaw: torch.Tensor) -> torch.Tensor:
        """roll-pitch-yaw (xyz) radians -> quaternion (wxyz)."""
    ...
    def _quat_wxyz_to_rotvec(quat_wxyz: torch.Tensor) -> torch.Tensor:
        """Quaternion (wxyz) -> rotation vector (axis * angle), radians."""
        q = quat_wxyz / (torch.norm(quat_wxyz, dim=-1, keepdim=True) + 1e-8)
        q = torch.where(q[:, :1] < 0.0, -q, q)  # shortest path
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # 5维动作 [dx, dy, dz, d_yaw, gripper] -> 6维 [dx, dy, dz, dRx, dRy, dRz]
        # 平移仍然由键盘控制；旋转则强制保持“竖直向下”(roll=pi, pitch=0)，并允许绕 Z 的 yaw 通过 d_yaw 微调。
        self.processed_actions[:, :3] = self.current_actions[:, :3] * self.action_scale

        # current yaw from ee quaternion (base frame)
        rpy_curr = self._quat_wxyz_to_rpy_rad(ee_quat_curr_b)
        yaw_curr = rpy_curr[:, 2]

        # yaw delta command (relative)
        ...
        # quaternion error -> rotation vector command (relative)
...
def _quat_wxyz_to_rpy_deg(quat_wxyz: torch.Tensor) -> torch.Tensor:
    """Convert quaternion (w, x, y, z) to roll-pitch-yaw in degrees.
    Returns tensor of shape (N, 3) in degrees.
    """
    # ensure float
...
def run_simulator(env: CylinderGraspingDemo):
    ...
    ee_pos_initial, ee_quat_initial = env._compute_frame_pose()
    r0, p0, y0 = _quat_wxyz_to_rpy_deg(ee_quat_initial)[0].detach().cpu().tolist()
    print(
        f"初始末端位置: [{ee_pos_initial[0,0]:.3f}, {ee_pos_initial[0,1]:.3f}, {ee_pos_initial[0,2]:.3f}] m | "
        f"姿态 quat(wxyz)=[{ee_quat_initial[0,0]:.4f}, {ee_quat_initial[0,1]:.4f}, {ee_quat_initial[0,2]:.4f}, {ee_quat_initial[0,3]:.4f}] | "
        f"rpy(deg)=[{r0:.1f}, {p0:.1f}, {y0:.1f}]"
    )
    ...
    gripper_action_units = 0.00  # start from a small opening (0.02 target after *0.5)
        ...
        while simulation_app.is_running():
            ...
            if step_counter % steps_per_half_sec == 0:
                try:
                    ...
                    def _contact_ratio(d, thr):
                        if d is None:
                            return torch.zeros((env.num_envs,), device=env.device)
                        dn = d.to(device=env.device, dtype=torch.float32) / 255.0
                        contact = (dn < thr).float()
                        return contact.mean(dim=(1, 2, 3), keepdim=False)
                    dl = env.gsmini_left.data.output.get("camera_depth") if hasattr(env, "gsmini_left") else None
                    dr = env.gsmini_right.data.output.get("camera_depth") if hasattr(env, "gsmini_right") else None
                    dmean_l = _depth_mean_norm(dl).mean().item()
                    dmean_r = _depth_mean_norm(dr).mean().item()
                    ratio_l = _contact_ratio(dl, thr).mean().item()
                    ratio_r = _contact_ratio(dr, thr).mean().item()
                    print(
                        f"[触觉深度] step {step_counter}: "
                        f"dmean_l={dmean_l:.4f}, dmean_r={dmean_r:.4f}, "
                        f"thr={thr:.3f}, ratio_l={ratio_l:.3f}, ratio_r={ratio_r:.3f}"
                    )
                except Exception:
                    pass
            ...
            if step_counter % 60 == 0:
                ...
                ee_pos_loop, ee_quat_loop = env._compute_frame_pose()
                r1, p1, y1 = _quat_wxyz_to_rpy_deg(ee_quat_loop)[0].detach().cpu().tolist()
                print(
                    f"[step {step_counter}] 末端夹爪位置: "
                    f"[{ee_pos_loop[0,0]:.3f}, {ee_pos_loop[0,1]:.3f}, {ee_pos_loop[0,2]:.3f}] m"
                )
                print(
                    f"[step {step_counter}] 末端姿态 quat(wxyz): "
                    f"[{ee_quat_loop[0,0]:.4f}, {ee_quat_loop[0,1]:.4f}, {ee_quat_loop[0,2]:.4f}, {ee_quat_loop[0,3]:.4f}] | "
                    f"rpy(deg)=[{r1:.1f}, {p1:.1f}, {y1:.1f}]"
                )
                print(
