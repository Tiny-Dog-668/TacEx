...
class CylinderGraspingDemoCfg(DirectRLEnvCfg):
    ...
    wrist_camera: CameraCfg = CameraCfg(
        ...
        offset=CameraCfg.OffsetCfg(
            pos=(0.25, 0, -0.12),      
            # 第一个参数可以改变相机在X方向上的位置，正值表示相机位于末端执行器前方，这里是为了在快抓住物体时，尽量让物体不要出现在画面中
            rot=(0.0, 0.0, 0.0, 1.0),
            convention="ros",
        ),
    )
...
def run_simulator(env: CylinderGraspingDemo):
    ...
    print("保持仿真运行：↑↓/W,S 控制 Z，上下；←→/A,D 控制 Y，左右；Q/E 控制 X 前后；P 保存腕部相机图片。按 Ctrl+C 退出。")
    ...
    print(f"[INFO] 按 P 保存腕部相机图片，输出目录: {wrist_image_output_dir.resolve()}")

    step_counter = 0
    manual_wrist_save_count = 0

    def _save_wrist_image(step_idx: int, tag: str):
        nonlocal wrist_images_saved, manual_wrist_save_count
        try:
            wrist_image_output_dir.mkdir(parents=True, exist_ok=True)
            ...
            save_idx = wrist_images_saved if tag == "auto" else manual_wrist_save_count
            filename = wrist_image_output_dir / f"wrist_{tag}_{save_idx:03d}_step_{step_idx:05d}_env{env_id}.png"
            try:
    ...
    def _maybe_save_wrist_image(step_idx: int):
        ...
        if not save_wrist_images or wrist_images_saved >= wrist_images_to_save:
            return
        _save_wrist_image(step_idx, tag="auto")
    ...
    try:
        while simulation_app.is_running():
            ...
            if pending["save_wrist"]:
                _save_wrist_image(step_counter, tag="manual")
                pending["save_wrist"] = False
